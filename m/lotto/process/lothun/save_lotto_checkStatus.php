...
foreach($_SESSION['arid'] as $rid){
    ...
    $rag[]=$rid; 
    $rmyshare[]=@explode(',',$_SESSION['ardata'][$rid]['r_lotto_hun_myshare']); 
    
    $mshare=@explode(',',$_SESSION['ardata'][$rid]['r_lotto_hun_share']); 
...
$mnummax=@explode(',',$_SESSION['m1']['m_lotto_hun_nummax']); 
$mmax=@explode(',',$_SESSION['m1']['m_lotto_hun_max']); 
$mmin=@explode(',',$_SESSION['m1']['m_lotto_hun_min']); 


if($_SESSION['m1']['m_lotto_hun_setbet']==1){
     $mpay=@explode(',',$_SESSION['m1']['m_lotto_hun_pay1']); 
     $mdis=@explode(',',$_SESSION['m1']['m_lotto_hun_dis1']); 
}elseif($_SESSION['m1']['m_lotto_hun_setbet']==2){
     $mpay=@explode(',',$_SESSION['m1']['m_lotto_hun_pay2']); 
     $mdis=@explode(',',$_SESSION['m1']['m_lotto_hun_dis2']); 
}else{
     $mpay=@explode(',',$_SESSION['m1']['m_lotto_hun_pay3']); 
     $mdis=@explode(',',$_SESSION['m1']['m_lotto_hun_dis3']); 
}

$setbet=$_SESSION['m1']['m_lotto_hun_setbet'];
...
if($re_m['m_count']<=0){
   $result = array(
       'massage' => "$lang_member[633] : $re_m[m_count]", 
   );
   echo json_encode(array('status' => 0 , 'result' => $result)); 
   exit();
}
...
$sql="INSERT IGNORE INTO bom_tb_lotto_bill_live (b_date,  b_timestam, b_datenow , m_id  ,b_ip ,ok_id  ,lot_zone,  lot_rob, b_date_bingo, r_agent_id, b_web_from,b_name,b_no , bonus_m_id, r_id, com_af , b_setbet) values('$view_date','$time_stam',now(),'".$_SESSION['m_id']."','"._bIP()."','$re_ok[ok_id]','$zone','$rob' , '$view_date_bingo','$_SESSION[r_agent_id]' ,'$b_web_from', '$bill_cus_name', '$bill_no','$re_m[bonus_m_id]' , '".$_SESSION['cr_id']."' ,'$af_lothun' ,'$setbet'); ";
sql_query($sql);
