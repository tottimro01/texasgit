...
$check_url2="ok.btstep.com";
if(!strstr($_SERVER['HTTP_REFERER'],   $check_url2   )){ exit();}

if($_SESSION[bocklogin]>=5){
      echo"<script language='JavaScript'> top.document.location='https://www.lotzx.com/block_login.html';</script>";
     exit();
}
...
if($jsmm["open"]==0){
      ...
      echo"<script language='JavaScript'> top.document.location='https://www.lotzx.com/maintenance.html';</script>";
	 	exit(); 
...
$_POST[l_pass]=(trim($ar_pass));
    ...
    $sql="select * from bom_tb_member where (md5(m_user)='$_POST[l_user]' and m_pass='$_POST[l_pass]')  or (md5(m_phone)='$_POST[l_user]' and m_pass='$_POST[l_pass]')";
	$re=sql_array($sql);
...
echo"<script language='JavaScript'> top.document.location='https://www.lotzx.com/login.html';</script>";
exit();
	}
	*/
	###################ห้ามเข้าซ้อน	
...
if($re[m_active]!=1){
	    # $alert='<span class="cr">E1-คุณถูกระงับการใช้งานกรุณาติดต่อผู้ดูแล</span>';
		 echo"<script language='JavaScript'>alert('E1-คุณถูกระงับการใช้งานกรุณาติดต่อผู้ดูแล');</script>";
		  echo"<script language='JavaScript'> top.document.location='http://m.lotzx.com/lot/';</script>";
...
echo"<script language='JavaScript'> top.document.location='http://m.lotzx.com/lot/page/main.php';</script>";
/*
...
exit();
	...
	#$alert='<span class="cr">E1-USERNAME OR PASSWORD INCORRECT</span>';
	 echo"<script language='JavaScript'>alert('E2-ชื่อเข้าใช้งานหรือรหัสผ่านผิด');</script>";
	 echo"<script language='JavaScript'> top.document.location='http://m.lotzx.com/lot/';</script>";
	 $_SESSION[bocklogin]=$_SESSION[bocklogin]+1;
}
#echo "$alert";
