	...
	if(lotto.length<=0){
		...
		if (rcc == true) {	
			$.ajax({
				...
				data: { "lotto": lotto , "tlot": 2 , "zone": 1 , "rob": 1 },
				dataType:"json",
			/*	timeout: 30000,*/
...
<div class="row">
  <div class="panel panel-default">
    ...
    <div class="panel-heading">
      <h1 class="panel-title thaisan" align="center" style="font-size:32px; color:#000"><strong><!-- 3 ตัวหน้า --><?=$lang_l3tuaNa;?></strong></h1>
    </div>
    <div class="panel-body">
      <div class="row">
        <div class="col-xs-3ths">
          <button class="btn btn-default btn-block thaisan" onClick="fnumcross()" style="margin-bottom:5px"><strong><i class="fa fa-repeat"></i> <!-- กลับ --><?=$lang_back;?></strong></button>
        </div>
        <div class="col-xs-3ths">
          <button class="btn btn-default btn-block thaisan"  onClick="fcopymoney(1,'tab1')" style="margin-bottom:5px" name="col1money" id="col1money"><strong><i class="fa fa-files-o"></i> <!-- คัดลอก --><?=$lang_copy;?></strong></button>
        </div>
        <div class="col-xs-3ths">
          <button class="btn btn-default btn-block thaisan" onClick="fcopymoney(2,'tab2')" style="margin-bottom:5px" name="col2money" id="col2money"><strong><i class="fa fa-files-o"></i> <!-- คัดลอก --><?=$lang_copy;?></strong></button>
        </div>
        <div class="col-xs-3ths">
          <button class="btn btn-default btn-block thaisan" onClick="fcopymoney(3,'tab3')" style="margin-bottom:5px" name="col3money" id="col3money" <?     if($profile["user_stabdown"]==0){echo' disabled="disabled"'; }?>><strong><i class="fa fa-files-o"></i> <!-- คัดลอก --><?=$lang_copy;?></strong></button>
        </div>
        <form name="lot3f" method="post" action="main.php?cmd=lot3f&subcmd=save" id="lot3f" onSubmit="return save_lotto();">
