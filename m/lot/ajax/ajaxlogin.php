<?php 
	...
	require('../../../geoiploc.php');
	require('../inc/data.php');
	require('../../../lib/Mobile-Detect/Mobile_Detect.php');
	$detect = new Mobile_Detect;  

	if(!strstr($_SERVER['HTTP_REFERER'], $check_url)){
		echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b>".$lang_member[2356]."</span>";
	 	exit();
	}

	################OPEN BET maintenance 
	$maintenance = getMaintenance();
	if($maintenance["open"]==0){
      	echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b>".$lang_member[1123]."</span>";
	 	exit(); 
	}

	$_POST['l_user'] = strtolower(trim(str_replace(array('@zx','@ZX'),array('',''),$_POST["tUser"])));
	$_POST['l_pass'] = md5(trim($_POST["tPsw"]));
	...
	if($re['m_id']!=""){
		...
		if($re['m_active']==3){
		    $alert="<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b>".$lang_member[59]."</span>";
		...
		$alert="<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b>".$lang_member[2250]."</span>";
	 	$_SESSION['bocklogin']=$_SESSION['bocklogin']+1;
	}
	echo "$alert";
?>
