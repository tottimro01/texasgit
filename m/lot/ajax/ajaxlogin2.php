...
if($_SESSION[bocklogin]>=5){
   echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[2354]."</span>"; //ระงับ!!! เนื่องจากเข้าระบบผิดเกินจำกัด
     exit();
}
	...
	if(trim($strUsername) == "")
	{
		echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[1904]."</span>"; //โปรดระบุชื่อผู้ใช้งาน
		exit();
	}
	
	if(trim($strPassword) == "")
	{
		echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[2179]."</span>"; //โปรดระบุรหัสผ่าน
		exit();
	}
...
if($jsmm["open"]==0){
		...
		echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[1123]."</span>"; //กำลังอยู่ในการ ซ่อมบำรุง ประมาน 45 นาที
		exit();
...
if($c_log>0){
echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[2355]."</span>"; //มีผู้ใช้งานอยู่ กรุณารอ 2 นาที
exit();
	}
	###################ห้ามเข้าซ้อน	
...
if($re[m_active]!=1){
		echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[1125]."</span>"; //คุณถูกระงับการใช้งานกรุณาติดต่อผู้ดูแล
		exit();
}else{
############################################
@session_start();
...
echo "<span style='color:#FFF'/><b><i class='fa fa-times-circle'></i></b> ".$lang_member[2250]."</span>";	//ชื่อผู้ใช้งาน หรือ รหัสผ่าน ไม่ถูกต้อง
