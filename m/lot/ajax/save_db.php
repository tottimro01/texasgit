...
if ($x_sum > 0) {
    if ($_SESSION[xcount] < $x_sum) {
		$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div></div><div class="col-xs-4" align="center"><span class="label label-danger">'.$lang_b[6]." : ".number_format($re_m[m_count]).'</span></div>';
    } else {
        $cmax = $hmax[$type_lot];
        $cmin = $hmin[$type_lot];
	...
	#########################JSON STOP#######################################		
	#########################JSON STOP#######################################							
$url_file_s="../../../json/lotto/stop/".$type_lot."_".$q_num.".json";							
if(file_exists($url_file_s)){
	
		################STOP
$url_file_sx="../../../json/lotto/stop/mid/".$_SESSION[mid]."_".$type_lot."_".$q_num.".json";	
if(!file_exists($url_file_sx)){
	
###########################สร้างใหม่
$mm_js=file_get_contents2($url_file_s);
...
$mm_js=file_get_contents2($url_file_sx);
$jstop = json_decode($mm_js, true);	
$jxstop=$jstop[0];	
...
$jxstop[number]="";
 ...
 if($time_stam>$jxstop[xnext] and $x_sum>$jxstop[sum] and $jxstop[number]!=""  ){
	 ...
	 ##########ซื้อแบบตัว
	 if($_SESSION['m_bet_tou']==1){
#	$_SESSION[error][1][]='<span class="cr">จำกัดเลข '.$q_num.' :  '. round($jxstop[sum]/14.29).'ตัว/ครั้ง</span>';		 
	$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">จำกัดเลข '.$q_num.' :  '.  round($jxstop[sum]/14.29).'ตัว/ครั้ง</span></div>';
		 }else{
	$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">จำกัดเลข '.$q_num.' :  '. $jxstop[sum].' บาท/ครั้ง</span></div>';
		 }
...
# $_SESSION[error][1][]='<span class="cr">จำกัดเลข'.$q_num.' :  ชั่วคราว</span>';	
		$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">จำกัดเลข '.$q_num.' : รอ '. _fbt($jxstop[xnext]-$time_stam).'นาที</span></div>';
...
# $_SESSION[error][1][]='<span class="cr">'.$q_num.' เหลือแทง  : '.number_format($jxstop[sum]).'</span>';	
	$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">'.$q_num.' เหลือแทง  : '.number_format($jxstop[sum]).'</span></div>';
...
if($sumck0<=0 and $sumck0!=""){
      $_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger"><b>'.$lang_lot[40].'</b></span></div>';
...
sql_query($sql);
 ...
 include("save_topmax.php");
  ########################ประวัติเลขเต็ม	
					...
					$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger"><b>'.$lang_lot[40]."</b> ".$txt_q.'</span></div>';
...
if($type_lot==3 or $type_lot==20  or $type_lot==24){			
			########################จำกัดต่อ 1 เลข
			$url_file_5a="../../../json/lotto/number/m_".$type_lot."_".$q_num3."_".$_SESSION['mid'].".json";
			...
			########################จำกัดต่อ 1 เลข
			$url_file_5a="../../../json/lotto/number/m_".$type_lot."_".$q_num."_".$_SESSION['mid'].".json";
			$sumck5a=jrlot($url_file_5a);
			########################จำกัดต่อ 1 เลข	
	}
...
$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger"><b>'.$lang_lot[54]."</b> ".$x_level[5].'</span></div>';
	}else{
					########################จำกัดต่อ 1 เลข	
	#############คิดนอกบอม
...
#######################################################LOG
	...
	#######################################################LOG
									...
									/*
									$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-success">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-success">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-success"> '.number_format($x_sum).' '.$lang_m[21] .'</span></div>';
								...
								$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-success">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-success">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-success"> '.number_format($x_sum/$x_sum9).' ตัว '.$lang_m[21] .'</span></div>';
								$_SESSION[count_sum]=$_SESSION[count_sum]+$x_sum;
				...
				$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">'.$lang_b[5]." ".number_format($cmax).'</span></div>';
            }
        } else {
			$_SESSION[error][1][] = '<div class="col-xs-4" align="center"><span class="label label-danger">' . $lot_type["th"][$type_lot].'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">' . _dt($q_num) .'</span></div><div class="col-xs-4" align="center"><span class="label label-danger">'.$lang_b[4]." ".number_format($cmin).'</span></div>';
        }
    } #	if($_SESSION[xcount]<$x_sum){
} #if($x_sum>0){
}#if($hpay[$type_lot]>0.00){
