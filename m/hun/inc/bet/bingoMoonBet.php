...
<div id="FullBody">
	...
	<div id="mainFullContain" style="background: #e0e0e0;">
      <table width="100%" id="bingo9data" data-role="none">
      	<tr id='liveVdeo'>
      		<td colspan="3">
      			<!-- <video id="vdoplayer" class="native" controls autoplay width="100%" height="200px" preload="metadata" webkit-playsinline="true" playsinline="true"> -->
...
function getLinkVdo()
{
	$.get('data/getVDO.php').done(function(data){
		
  		var vdo = document.getElementById('vdoplayer');
		var link = convertVdoUrl(data['VDO']);
		vdo.setAttribute('src', link);
		console.log(link)
	})
}

 function convertVdoUrl(originalUrl)
  {
    var _protocal = originalUrl.substring(0, originalUrl.indexOf(':'))
    var _newUrl;
    if(_protocal == 'rtmp')
    {
      originalUrl = originalUrl.replace(_protocal, 'http');
      originalUrl += '/index.m3u8';
      _newUrl = document.createElement('a')
      _newUrl.href = originalUrl;
      _newUrl.port = '81';
       return _newUrl.href;
    }else{
      _newUrl = originalUrl;
...
function selectRow(e,cerrentTepe,colId)
{
	...
	}else if(cerrentTepe=='2')
	{
		...
		for(var j =0; j<trLength;j++)
		{
			...
			if(cell1Value.length==1 || cell1Value.length==2)
			{
...
function checkZoneOpen()
{
	...
	// timeMil = '1503392280';
		...
		{
			window.location = "http://m.lotzx.com/hun/index.php?p=bingoMoonBet&zone=3";
		}
...
function save_lotBet()
{
	...
	for(var i=0; i<6;i++)
	{
		...
		if(_txt1!='')
		{
			...
			_txt+= price1;
			_txt+='-';
			var price2=$('#BetInputBody').children('tr:eq( '+i+' )').children('td:eq( 2 )').children('input').val();
...
$(document).on('click','#BtncolId_0',function(){
	...
	for(var i=0; i<unique.length; i++)
	{
		...
		if(unique[i].length==1 || unique[i].length==2)
		{
			...
			$('.bingoNumber:eq( '+i+' )').closest('tr').children('td:eq( 2 )').children('input').val('');		
...
$(document).on('click','#BtncolId_3',function(){
	...
	if($(this).hasClass('readonly'))
	{
		...
		var col3Value  = $('.BetInput:eq( 0 )').children('td:eq( 2 )').children('.colId_3').val(); //ข้อมูลของแถวแรกที่จะคัดรอก
		if(col3Value!='')
		{
