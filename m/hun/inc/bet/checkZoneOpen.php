	...
	$.post("data/getZoneClose.php",{"zone":"<?php echo $_GET['zone']; ?>"}).done(function(data){
		...
		if(timeMil<=data['o_limit_time']) // big เปิดทั้งหมด
		{
			if(zone=='1')
			{
				...
				window.location = "http://m.lotzx.com/hun/index.php?p=bingo9BetOpen";	
			}else if(zone=='2')
			{
				
				window.location = "http://m.lotzx.com/hun/index.php?p=bingoMchBetOpen";	
			}
			else if(zone=='3')
			{
				
				window.location = "http://m.lotzx.com/hun/index.php?p=bingoMoonBetOpen";	
			}
		}else
		{
			$('#closeBet').text('');
