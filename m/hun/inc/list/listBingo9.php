...
<table width="100%" id="MtableBingo9"> 
	<tr>
		<td>
			<div id="SwitchBillLayout">
      			<div class="boxRightContainer">
      			    <div class="billBtn1_9 billBtn1 ">ดูแบบรายบิล</div>
      			    <div class="billBtn2_9 billBtn2 billactive" onclick="MoonListactive9(2);">ดูทั้งหมด</div>
...
function setDefult9()
{
	getBillData9($('#DateselectBox').val());

	$('#MtableBingo9 .boxRightContainer div').removeClass('billactive');
	if(sessionStorage.getItem('listbingoMoon')==1)
	{
		$('#MtableBingo9 .boxRightContainer .billBtn2_9').addClass('billactive');
		$('.billBtn1_9').removeAttr('onclick');
		$('.billBtn2_9').attr('onclick','MoonListactive9(2);');

	}else{
		
		$('#MtableBingo9 .boxRightContainer .billBtn1_9').addClass('billactive');
		$('.billBtn2_9').removeAttr('onclick');
		$('.billBtn1_9').attr('onclick','MoonListactive9(1);');
	}
}
	
//ค่าเร่มต้น
	function MoonListactive9(type)
	{
	...
	function showBillData9(BillData)
	{
		...
		//มีข้อมูลในตารางหรือไม่ ถ้ามีให้แสดงผลรวม
		...
		$('#showListTable9').append(htmlText);	
		$('#loading').hide();		
	}

	function showBillAllData9(BillAllData)
	{
