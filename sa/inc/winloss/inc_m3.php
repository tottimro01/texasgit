...
sum(
   	ROUND( 	( - br_bonus_".$ag_level."  )*(( $keep_share )/100) ,10)
   ) as w3   ,
sum(
...
END
 ...
 from bom_tb_football_bill where m_id = '".$rs['m_id']."' and b_confirm != 1  and b_accept=1  and b_status!=5   and b_numstep=1 and  b_zone=6     and   STR_TO_DATE(b_date,'%d-%m-%Y') BETWEEN '$edate' and '$ddate'";
$rs1=sql_array($sql);#and bill_id=101218
if($cb_sc!=""){
...
END
 ...
 from bom_tb_football_bill where m_id = '".$rs['m_id']."' and b_confirm != 1  and b_accept=1  and b_status!=5   and b_numstep=1 and  b_zone!=1   and  b_zone!=6  and   STR_TO_DATE(b_date,'%d-%m-%Y') BETWEEN '$edate' and '$ddate'";
$rs11=sql_array($sql);
if($cb_sp!=""){
$b0=$rs11['t1'];
...
END
 ...
 from bom_tb_football_bill where m_id = '".$rs['m_id']."' and b_confirm != 1  and b_accept=1  and b_status!=5   and b_numstep=1 and  b_zone=1       and   STR_TO_DATE(b_date,'%d-%m-%Y') BETWEEN '$edate' and '$ddate'";
$rs12=sql_array($sql);
if($cb_bx!=""){
$c0=$rs12['t1'];
...
END
 ...
 from bom_tb_football_bill where m_id = '".$rs['m_id']."' and b_confirm != 1  and b_accept=1  and b_status!=5   and b_numstep>1     and   STR_TO_DATE(b_date,'%d-%m-%Y') BETWEEN '$edate' and '$ddate' ";
$rs13=sql_array($sql);
if($cb_st!=""){
$d0=$rs13['t1'];
...
sum(
   	ROUND( 	( -b_bonus   )*(( $keep_share )/100) ,10)
   ) as w3   ,
sum(
   	ROUND( 	( b_pay-b_total  )*(( $keep_share )/100) ,10)
   ) as w2   , 
   sum(
...
// $sum2=(($a2+$b2+$c2+$d2)+($e2+$f2+$g2)+$h2+$i2+$sumafr)*100;
...
$sum2=(($a2+$b2+$c2+$d2)+($e2+$f2+$g2)+$h2+$i2+$sumafr);
