...
<table class="table table-striped table-bordered table-hover" id="table_data">
    ...
    <tbody>
        ...
        $no         = 1;
$sum        = 0;
$sql_bill   = sql_query("select * from bom_tb_football_bill where m_id = '$re_member[m_id]' and b_confirm != 1  and b_zone <> 6 and b_zone <> 1 and b_numstep = 1 and b_status <> 5   and   STR_TO_DATE(b_date,'%d-%m-%Y') BETWEEN '$edate' and '$ddate' order by bill_id desc");
