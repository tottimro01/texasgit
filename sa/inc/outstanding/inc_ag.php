...
if( $gametype=="sc"){
	...
	$where="bom_tb_football_bill_live where r_agent_id like '%*".$rs['r_id']."*%' and b_confirm != 1   and b_accept=1  and b_status=5 and b_numstep=1 and  b_zone=6";
	$r33=" br_pay_".$ag_level." - br_pay_".$r_level." ";	
}elseif( $gametype=="sp"){
	$total="b_sum";
	$dis1=" br_pay_".$r_level." ";
	$where="bom_tb_football_bill_live where r_agent_id like '%*".$rs['r_id']."*%' and b_confirm != 1  and b_accept=1  and b_status=5 and b_numstep=1 and  b_zone!=1 and  b_zone!=6";
	$r33=" br_pay_".$ag_level." - br_pay_".$r_level." ";	
}elseif( $gametype=="bx"){
	$total="b_sum";
	$dis1=" br_pay_".$r_level." ";
	$where="bom_tb_football_bill_live where r_agent_id like '%*".$rs['r_id']."*%'  and b_confirm != 1 and b_accept=1  and b_status=5 and b_numstep=1 and  b_zone=1 ";
	$r33=" br_pay_".$ag_level." - br_pay_".$r_level." ";		
}elseif( $gametype=="st"){
	$total="b_sum";
	$dis1=" br_pay_".$r_level." ";
	$where="bom_tb_football_bill_live where r_agent_id like '%*".$rs['r_id']."*%' and b_confirm != 1  and b_accept=1  and b_status=5 and b_numstep>1  ";
	$r33=" br_pay_".$ag_level." - br_pay_".$r_level." ";			
}elseif( $gametype=="lg"){
	$total="b_total";
...
sum(
   	ROUND( 	-( $total )*((b_myshare_".$r_level.")/100) ,10)
   ) as r2 ,  
sum(
   	ROUND( 	(  $dis1 )*((b_myshare_".$r_level.")/100) ,10)
   ) as r3  ,
sum(
