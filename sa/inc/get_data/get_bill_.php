 ...
 //  }
  ...
  if($_GET[mid]!=""){ $xmid="and m_id='$_GET[mid]'";}
	...
	if($_GET[orderby]==1){  // ตามชนะ หรือ เวลา
			$sql_num = "SELECT count(bill_id) as num FROM `bom_tb_football_bill_live` where 1 $c_date   $type $bet  $shop  $qq   $xmid $xbill  $b_out";
	}else{
		  $sql_num = "SELECT count(bill_id) as num FROM `bom_tb_football_bill_live` where 1 $c_date   $type $bet  $shop  $qq   $xmid $xbill  $b_out";
	}
  ...
  // แบ่งหน้า    
	...
	if($_GET[orderby]==1){  // ตามชนะ หรือ เวลา
			$sql_bill = "SELECT * FROM `bom_tb_football_bill_live` where 1 $c_date   $type $bet  $shop  $qq   $xmid $xbill  $b_out ORDER BY `bill_id` desc LIMIT {$start} , {$rowPerPage}";

	}else{
		  $sql_bill = "SELECT * FROM `bom_tb_football_bill_live` where 1 $c_date   $type $bet  $shop  $qq   $xmid $xbill  $b_out ORDER BY b_status asc,bill_id desc LIMIT {$start} , {$rowPerPage}";
	}
						...
						<?php 
								if($rs[b_accept]==1){
									...
									?>
										...
										<a href="?p=<?=$_GET[p];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&b=<?=$_GET[b];?>&mid=<?=$_GET[mid];?>&pg=<?=$_GET[pg];?>&d=<?=$_GET[d];?>&view=<?=$_GET[view];?>&q=<?=$_GET[q];?>&orderby=<?=$_GET[orderby];?>&amp;ac=accept&amp;act=1&amp;id=<?=$rs[bill_id];?>&amp;mid=<?=$rs[m_id];?>"  onclick="return confirm('ยืนยันยกเลิกรับตั๋ว');"><img src="assests/img/1.png" /></a>
									<?
								}else{
