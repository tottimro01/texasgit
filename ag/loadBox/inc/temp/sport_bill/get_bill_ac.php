 ...
 $is_delete_bin = true;
  ...
  if($_GET[mid]!=""){ $xmid="and m_id='$_GET[mid]'";}
	...
	if($_GET[orderby]==1){  // ตามชนะ หรือ เวลา
			$sql_num = "SELECT count(bill_id) as num FROM `bom_tb_football_bill_live` where 1 $c_date and (b_accept=0 or b_accept=3) and m_accept=0  and r_agent_id = '$r_agent_id' $type $bet  $shop  $qq   $xmid $xbill  $b_out";
	}else{
		  $sql_num = "SELECT count(bill_id) as num FROM `bom_tb_football_bill_live` where 1 $c_date  and (b_accept=0 or b_accept=3) and m_accept=0  and r_agent_id = '$r_agent_id'  $type $bet  $shop  $qq   $xmid $xbill  $b_out";
	}
