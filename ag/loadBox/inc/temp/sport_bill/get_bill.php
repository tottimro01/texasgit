 ...
 $is_delete_bin = true;
  ...
  if($_GET[mid]!=""){ $xmid="and m_id='$_GET[mid]'";}
	...
	if($_GET[orderby]==1){  // ตามชนะ หรือ เวลา
			$sql_num = "SELECT count(bill_id) as num FROM `bom_tb_football_bill_live` where 1 $c_date  and r_agent_id = '$r_agent_id' $type $bet  $shop  $qq   $xmid $xbill  $b_out";
	}else{
		  $sql_num = "SELECT count(bill_id) as num FROM `bom_tb_football_bill_live` where 1 $c_date  and r_agent_id = '$r_agent_id'  $type $bet  $shop  $qq   $xmid $xbill  $b_out";
	}
  ...
  // แบ่งหน้า    
	...
	if($_GET[orderby]==1){  // ตามชนะ หรือ เวลา
			$sql_bill = "SELECT * FROM `bom_tb_football_bill_live` where 1 $c_date  and r_agent_id = '$r_agent_id'  $type $bet  $shop  $qq   $xmid $xbill  $b_out ORDER BY `bill_id` desc LIMIT {$start} , {$rowPerPage}";

	}else{
		  $sql_bill = "SELECT * FROM `bom_tb_football_bill_live` where 1 $c_date  and r_agent_id = '$r_agent_id'  $type $bet  $shop  $qq   $xmid $xbill  $b_out ORDER BY b_status asc,bill_id desc LIMIT {$start} , {$rowPerPage}";
	}
						...
						<?php 
								...
								if($rs[b_accept]==1){
									...
									?>
										 ...
										 <?php /* ?>
										<a href="?p=<?=$_GET[p];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&b=<?=$_GET[b];?>&mid=<?=$_GET[mid];?>&pg=<?=$_GET[pg];?>&d=<?=$_GET[d];?>&view=<?=$_GET[view];?>&q=<?=$_GET[q];?>&orderby=<?=$_GET[orderby];?>&amp;ac=accept&amp;act=1&amp;id=<?=$rs[bill_id];?>&amp;mid=<?=$rs[m_id];?>"  onclick="return confirm('ยืนยันยกเลิกรับตั๋ว');"><img src="img/1.png" /></a>*/?>
										
										<?=$f_accept[$rs[b_accept]];?>
										<span onclick="bill_action('?p=<?=$_GET[p];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&b=<?=$_GET[b];?>&mid=<?=$_GET[mid];?>&pg=<?=$_GET[pg];?>&d=<?=$_GET[d];?>&view=<?=$_GET[view];?>&q=<?=$_GET[q];?>&orderby=<?=$_GET[orderby];?>&amp;ac=accept&amp;act=1&amp;id=<?=$rs[bill_id];?>&amp;mid=<?=$rs[m_id];?>&perpapge=<?=$_GET[perpapge];?>','<?=$lang_ag[2135];?>')">
    				...
    				<? 	if($rs[br_bonus_1]>0 )
    						{
    							...
    							if($rs[b_admin]==0)
    							{
    								?>
    								<?/*
	 									<a href="?p=<?=$_GET[p];?>&d=<?=$_GET[d];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&b=<?=$_GET[b];?>&pg=<?=$_GET[pg];?>&view=<?=$_GET[view];?>&shop=<?=$_GET[shop];?>&mid=<?=$_GET[mid];?>&orderby=<?=$_GET[orderby];?>&amp;ac=pay&amp;id=<?=$rs[bill_id];?>&amp;ss=<?=$rs[b_admin];?>"  title="จ่ายเงิน" >
										<img src="assets/img/<?=$rs[b_admin];?>.png" alt="จ่ายเงิน" />
										</a>*/?>
										
										<span onclick="bill_action('?p=<?=$_GET[p];?>&d=<?=$_GET[d];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&b=<?=$_GET[b];?>&pg=<?=$_GET[pg];?>&view=<?=$_GET[view];?>&shop=<?=$_GET[shop];?>&mid=<?=$_GET[mid];?>&orderby=<?=$_GET[orderby];?>&amp;ac=pay&amp;id=<?=$rs[bill_id];?>&amp;ss=<?=$rs[b_admin];?>&perpapge=<?=$_GET[perpapge];?>','<?=$lang_ag[2136];?>')">
     			 ...
     			 <? 		}else{
					...
					<td align="center">
						 ...
						 <?  if($rs[b_status]==5 and $rs[b_accept]==1 and $is_delete_bin != false){?>
  							...
  							<?/*
    						<a href="?p=<?=$_GET[p];?>&d=<?=$_GET[d];?>&b=<?=$_GET[b];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&pg=<?=$_GET[pg];?>&view=<?=$_GET[view];?>&mid=<?=$_GET[mid];?>&shop=<?=$_GET[shop];?>&orderby=<?=$_GET[orderby];?>&pg=<?=$_GET[pg];?>&q=<?=$_GET[q];?>&amp;ac=del&amp;id=<?=$rs[bill_id];?>&amp;cc=<?=$rs[b_total];?>&amp;xmid=<?=$rs[m_id];?>"  onclick="return confirm('ยืนยันการยกเลิก');">
    									<img src="assets/img/false.png" />
    						</a>*/?>


							<span onclick="bill_action('?p=<?=$_GET[p];?>&d=<?=$_GET[d];?>&b=<?=$_GET[b];?>&type=<?=$_GET[type];?>&bill=<?=$_GET[bill];?>&pg=<?=$_GET[pg];?>&view=<?=$_GET[view];?>&mid=<?=$_GET[mid];?>&shop=<?=$_GET[shop];?>&orderby=<?=$_GET[orderby];?>&pg=<?=$_GET[pg];?>&q=<?=$_GET[q];?>&amp;ac=del&amp;id=<?=$rs[bill_id];?>&amp;cc=<?=$rs[b_total];?>&amp;xmid=<?=$rs[m_id];?>&perpapge=<?=$_GET[perpapge];?>','<?=$lang_ag[10];?>')">
								<img src="assets/img/false.png" />
    					</span>
