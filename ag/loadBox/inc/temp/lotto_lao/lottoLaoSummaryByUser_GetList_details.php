...
$sql="select * from bom_tb_lotto_ok  where lot_zone=3 and  lot_rob=1 and ok_date='$d'   order by ok_id desc limit 1";
$re_ok=sql_array($sql);
...
if(strpos($mid,"rid")!== false){
  ...
  (
    ...
    (
   	ROUND( 	-( br_bonus_".$ag_level.")*((b_myshare_".$ag_level.")/100) ,10)
   ) as c4 
   
  from bom_tb_lotto_bill_play_live where    ok_id='$ok_id'  and r_agent_id like '%*$rer[r_id]*%'    and b_accept=1  and lot_type=31    $aa order by lot_type asc , play_number asc  limit 20000   ";
$re=sql_query($sql);
while($rs=sql_fetch($re)){
  ...
  $data["val"][] = array(
         "mid" => $rer["r_id"],
         "muser" => $rer["r_user"],
         "number" => "".$rs["play_number"]." : [".$lotHun_typeArry[$rs["lot_type"]]."]",
         "user" => $rer["r_user"],
...
$sql="select 
  ...
  from bom_tb_lotto_bill_play_live where    ok_id='$ok_id'  and m_id = '$mid'    and b_accept=1 and lot_type=31  $aa order by lot_type asc , play_number asc  limit 20000   ";
$re=sql_query($sql);
while($rs=sql_fetch($re)){
