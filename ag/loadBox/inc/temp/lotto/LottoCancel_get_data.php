...
$sql="select * from bom_tb_lotto_ok  where lot_zone=1 and  lot_rob=1 and o_limit_time>'$time_stam'  order by ok_id desc limit 1";
$re_ok=sql_array($sql);
...
$sql="select 
  ...
  from bom_tb_lotto_bill_play_live where    ok_id='$ok_id'  and r_agent_id like '%*$r_id*%' and r_id != '$r_id'    and b_accept=1 and play_status=7  $ww  order by play_id desc    limit 500   ";
$re=sql_query($sql);
while($rs=sql_fetch($re)){
$sum10a=$rs[r1];
$sum10b=-$rs[r2];
$sum10c=$rs[r1]-$rs[r2];
 ...
 $sql="select 
  ...
  from bom_tb_lotto_bill_play_live where    ok_id='$ok_id'   and r_id = '$r_id'    and b_accept=1 and play_status=7  $ww  order by play_id desc    limit 500   ";
$re=sql_query($sql);
while($rs=sql_fetch($re)){
$sum10a=$rs[r1];
$sum10b=-$rs[r2];
