...
$sum_winloss = (@array_sum($asum1[1])-@array_sum($asum1[2])) + (@array_sum($asum2[1])-@array_sum($asum2[2]));
 ...
 $sql="select r_count  from bom_tb_agent where  r_id= '".$r_id."'";
  ...
  $sql = "select sum(m_count) as xtotal , m_count from bom_tb_member where  r_id = '".$r_id."' and m_id!='".$m_id."'";
  $re5 = sql_array($sql);
  ...
  // เครดิตทั้งหมดที่มี ลบกับ  เครดิตที่ให้ member 
 ...
 # $x_count3 = ($count_ag)-$x_count_total; // เครดิตทั้งหมดที่มี ลบกับ  เครดิตที่ให้ member และ agent
  ...
  // echo "เครดิตที่ได้ :".$re4["xtotal"]."<br>";
