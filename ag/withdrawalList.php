...
<?php 
  ...
  while($rs=sql_fetch($sql_list_tran)){
  	...
  	$list .= "<tr>
  				...
  				<td align=\"center\" >เครดิตปัจุบัน = <b class=\"cbu\">".number_format($m_count[$rs[m_id]],2)."</b><br> เครดิตรวมถอน = <b class=\"cr bb\">".number_format($m_count[$rs[m_id]]+$rs[t_count],2)."</b></td>
  				<td align=\"center\" >".$ab_bank[$rs[t_bank]]."<br><span class=\"cb\">".$ex[1]."</span></td>
  				<td align=\"center\"><b class=\"cr\">"._cbn(-$rs[t_count],2)."</b><br><span class=\"cb\">".$ex[0]."</span><br><span class=\"cbu\">".$ex[2]."</span></td>
    			<td align=\"center\">".$ab_status[$rs[t_status]]."</td>
    			<td align=\"center\"><input type=\"button\" value=\" $lang_ag[2127] \"  style=\"background:#0F6;  margin:5px;\" onclick=\"set_list(this,'ok','".$rs[m_id]."','".$rs[t_count]."','".$rs[t_id]."','".$rs[t_bank]."','".$rs[t_active]."');\" /><br>
    			<input name=\"Button\" type=\"button\" value=\" $lang_ag[4] \"  style=\"background:#F63;  margin:5px;\" onclick=\"set_list(this,'can','".$rs[m_id]."','".$rs[t_count]."','".$rs[t_id]."','".$rs[t_bank]."','".$rs[t_active]."');\"/></td>
