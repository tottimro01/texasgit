...
if($_GET[orderby]==1){  // ตามชนะ หรือ เวลา
			$sql_bill = "SELECT * FROM `bom_tb_football_bill` where b_confirm != 1 $c_date  and r_agent_id = '$r_agent_id'  $type $bet  $shop  $qq   $xmid $xbill  $b_out ORDER BY `bill_id` desc LIMIT {$start} , {$rowPerPage}";

	}else{
		  $sql_bill = "SELECT * FROM `bom_tb_football_bill` where b_confirm != 1 $c_date  and r_agent_id = '$r_agent_id'  $type $bet  $shop  $qq   $xmid $xbill  $b_out ORDER BY b_status asc,bill_id desc LIMIT {$start} , {$rowPerPage}";
	}
							...
							<div style="float:right;">
 							...
 							?> 
									<div>
											...
											<?php 
													$sql="select * from bom_tb_football_bill_play  where bill_id='$rs[bill_id]'   group by play_id order by play_id asc";
