            ...
            $ag_num_row = 0;
              ...
              ###################  load agent ################
               ...
               while ($rs = sql_fetch_as($re)){
			             ...
			             include("inc_m1.php");
			             ######################
		               if($sum0>0.00 or $sum0af>0.00){
                           $name = $rs['r_user']."^".$rs['r_name']."^A";
                           $member_name[] = $name;
                           $val[] = array($name, "$sum0","$sum1","$sum2","$sum3","$sum4","$sum5","$sum6","$sum7","$sum8","$sum9","$sum10");  // ข้อมูลในตาราง แก้ตรงนี้ 
                           $c++;
		               }
        ...
        echo json_encode($datas);
            ...
            $data = array();
             foreach($list_type as $key => $value){
				...
				include("inc_m5.php");
				######################
               $data['data'][$key] =array($value, "$sum0","$sum1","$sum2","$sum3","$sum4","$sum5","$sum6","$sum7","$sum8","$sum9","$sum10");
				 }
             }
