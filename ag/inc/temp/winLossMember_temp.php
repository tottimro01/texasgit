...
<script type="text/javascript">
  ...
  $( "#begin_date,#end_date" ).datepicker('setEndDate', "0");
                    ...
                    if(response.result.data){
                        ...
                        $.each(response.result.data,function(key,val){
                            ...
                            switch (response.result.listtype){
                                    ...
                                    tbody += '<tr>';
                                        $.each(val,function(k,v){
                                            ...
                                            if(k == 0){
                                                ...
                                                if(obj_name[2] == 'A'){
                                                    utype = '[<i class="ace-icon fa fa-users bigger-140 bg-icon"></i>] ';
                                                }else{
                                                    utype = '[<i class="ace-icon fa fa-user bigger-140 bg-icon"></i>] ';
                                                }
                                                tbody += '<td class="text-center">'+ ii +'</td>';
                                                tbody += '<td class="text-left"><a href="#"  class="b_color" onclick="getData(&quot;'+ obj_name[0] +'&quot;,&quot;'+ lv +'&quot;,&quot;'+ ulist +'&quot;,'+page+',\'Intable\');">'+ utype + obj_name[0] +' '+ obj_name[1] +'</a></td>';
                                            }else{
                                ...
                                case 'gameList' :
                                    col_name = "<?=$lang_ag[477];?>";
                                    tbody += '<tr>';
                                        $.each(val,function(k,v){
                                            var ulist =  response.result.userlist + '|' + lv +'_'+ v;
                                            if(k == 0){
                                                tbody += '<td class="text-center">'+ ii +'</td>'+
                                                         '<td class="text-left"><a href="#" onclick="getBill(&quot;'+ ubill +'&quot;,&quot;'+ v +'&quot;,50,1,&quot;'+ ulist +'&quot;);">'+ bet_type[v]+'</a></td>';
                        ...
                        var sl = (sum.length == 0) ? 12 : sum.length;
                        tfoot += '<tr><td colspan="2" class="text-strong"><?=$lang_ag[197];?> </td>';

                        for (var i = 1; i < sl; i++){
                            var footer_sum = 0;
                            if (isNaN(Number(sum[i]).toFixed(2)))
                            {
                                footer_sum = 0;
                            }else{
                                footer_sum = Number(sum[i]).toFixed(2);
                            }
                            tfoot += '<td class="text-right text-strong fnum" >'+ footer_sum +'</td>';
                        };
                        tfoot += '</tr>';
                        ...
                        table = '<thead>'+
                                    ...
                                    '</tr>'+
                                    '<tr>'+
                                        '<th><?=$lang_ag[190];?></th><th><?=$lang_ag[210];?></th><th><?=$lang_ag[248];?></th><th><?=$lang_ag[197];?></th>'+
                                        '<th><?=$lang_ag[190];?></th><th><?=$lang_ag[210];?></th><th><?=$lang_ag[248];?></th><th><?=$lang_ag[197];?></th>'+
    ...
    function getBill(user,game,no,page,ulist,zone = ""){
        $('#reloadWinLoseMember').load('show');

        var table    = "";
        var u        = user.split('_');
        var tag_game = "";
        var cn_zone = <?=json_encode($lang_g["casino_share"]);?>;
        ...
        $.ajax({
            ...
            if(zone != "")
            {
                tag_game += '<span class="label label-success arrowed-right arrowed-in cur" onclick="getBill(&quot;'+ user +'&quot;,&quot;'+ game +'&quot;,50,1,&quot;'+ulist+'&quot;);">'+bet_type[game] +'</span>';

                tag_game += '<span class="label label-success arrowed-right arrowed-in cur" onclick="getBill(&quot;'+ user +'&quot;,&quot;'+ game +'&quot;,50,1,&quot;'+ulist+'&quot;,'+zone+');">'+cn_zone[zone] +'</span>';
            }else{
                tag_game += '<span class="label label-success arrowed-right arrowed-in cur" onclick="getBill(&quot;'+ user +'&quot;,&quot;'+ game +'&quot;,50,1,&quot;'+ulist+'&quot;);">'+bet_type[game] +'</span>';
            }
...
function loadLiveScore(b_id){
    ...
    $.ajax({
      url: 'inc/temp/winLossMember/get_livescore.php',
      type: 'get',
      dataType: 'html',
      data: {b_id: b_id},
    })
