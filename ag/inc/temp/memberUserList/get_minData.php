...
// ดึงข้อมูล agent และ member ภายใต้ ที่มีค่าน้อยที่สุด
    ...
    $r_lotto_min_max = (min($check_r_lotto_min[1]) == "") ? 0 : min($check_r_lotto_min[1]);
    ...
    $r_lotto_hun_min_max = (min($check_r_lotto_hun_min[1]) == "") ? 0 : min($check_r_lotto_hun_min[1]);
    ...
    foreach ($check_r_lotto_hun_set_price as $key => $value) {
      $lotto_hun_set_price[$key] = min($value);
    }

    $game_share_max = max($check_r_games_share[1]);
    $game_com_max = max($check_r_games_dis[1]);
    $game_max_max = (max($check_r_games_max[1]) == "") ? 0 : max($check_r_games_max[1]);
    $game_min_max = (min($check_r_games_min[1]) == "") ? 0 : min($check_r_games_min[1]);
