  ...
  if($num_user == 0)
  {
    ...
    if($_count <= floatval(removeComma($x_count3)))
    {
      ...
      if($rs['rs'])
      {
        ...
        $sql2 = "update IGNORE `bom_tb_member` SET `m_name`='$name',`m_phone`= '$phone' ,`m_user`= '$user' ,`m_pass`= '$pass',`m_regis`= now(),`m_count`= '$_count' ,`m_count_de`= '$_count',`m_b_code`= '$m_b_code' ,`m_b_bank`= '$m_b_bank' ,`m_b_name`= '$m_b_name' ,`m_b_pass`= '$m_b_pass' ,`m_user_set`= NULL WHERE m_id = $last_id";
        sql_query($sql2);
