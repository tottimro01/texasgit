...
// ดึงประวัติการเงิน 
$sql_payment = "select * from bom_tb_all_payment where `m_agent_id` IN ('".implode('\',\'', $up_r_agent_id)."') and bap_type IN (5,6)  and `m_id` IS NOT NULL and bap_date between '$dsearch' and '$esearch'";
$result_payment = sql_query($sql_payment);

while($row = sql_fetch_as($result_payment))
{
  $ex_r_agent_id = explode("*", $row["m_agent_id"]);
