...
$sql="select * from bom_tb_lotto_bill_play_live   where bill_id='".$_POST["tbid"]."'   and (r_agent_id like '%*297*%'  or r_agent_id like '%*417*%'  or r_agent_id like '%*124*%' )";		
$re=sql_query($sql);
...
if($ok_del==1){
	$acc[]=1;	
$sql="update bom_tb_lotto_bill_play_live set b_accept=0, play_status=6, can_date=now() , can_by='joy_can'  where play_status=7 and play_id='$recan[play_id]'";   
sql_query($sql);	
