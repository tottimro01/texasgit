...
// echo"<script language='JavaScript'>alert('คุณไม่สามารถเข้าใช้งานระบบได้');</script>";
*/
// 	 exit();
// }
/*
...
$_POST['l_pass']=md5(trim($_POST['l_pass']));
...
/* echo"<script language='JavaScript'>alert('break');</script>";*/
// exit();
 $location=getCountryFromIP($ipc, " NamE ");
  
   $sql="update IGNORE  bom_tb_member set m_login=now(),m_ip='$ip',m_country='$location', m_token='$m_token'  where m_id='".$_SESSION['m_id']."'";
   sql_query($sql);
...
/* echo"<script language='JavaScript'> top.document.location='http://2jzgte.".$check_url."/check_login_link.php?token=".$_SESSION['m_token']."&id=".$_SESSION['m_id']."&user=".$_SESSION['m_user']."';</script>";*/
echo"<script language='JavaScript'>var success = 1;  window.location.href='http://betwin.".$check_url."/check_login_link.php?token=".$_SESSION['m_token']."&id=".$_SESSION['m_id']."&user=".$_SESSION['m_user']."&clang=".$_SESSION['lang']."&clink=".$_SESSION['m_login_from']."&ref_url=".$ref_url."';</script>";
exit();

	
}
}else{
	$sql="select * from bom_tb_member where (m_user='".$_POST['l_user']."') or (m_user_set='".$_POST['l_user']."')  or (m_phone='".$_POST['l_user']."')";
	$re=sql_array($sql);
	if($re!== null){
		######################LOG##################
		include("geoiploc.php"); // Must include this
