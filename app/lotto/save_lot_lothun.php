...
$recx=@explode(',',$rec['con_open_lotto_hun_new']);
if($recx[$zone]==0){
    $arr["Status"]     = "2";
    $arr["Msg"]        = $lang_member[1124]; //"ปิดรับพนัน";
    $arr["Licen"]      = "SC";
...
$setbet=$re_m['m_lotto_hun_setbet'];
...
if ($count_step == 0) {
	...
	*/
    ...
    $arr["txtBillId"] = $name_bill." ".$lang_member[706]." ".$reeb[bill_id];
    ...
    if($_POST[zone]==3 and $ttlot==5){
        $arr["txtTotal"] = $lang_member[559]." ".number_format($_SESSION[count_sum3t])." ".$lang_member[1183];
    }else{
