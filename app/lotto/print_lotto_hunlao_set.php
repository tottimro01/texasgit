...
if($zone==18){
		...
		if($lot_zone_bet[$zone]>1){
			...
			$name_bill = $lang_g['lotZone'][$zone].'  '.$lang_g["laoset"];
		}
	}
...
$im  = imagecreatetruecolor(380, ($numstep*37)+$page_hight);		
...
imagefilledrectangle($im, 0, 0, 380, ($numstep*37)+$page_hight, $white);


 #'.$lot_txt[1].'

$n_1 =$lot_txt[2].' :  ['.$re_r[r_user].']';
$n_2 =$lot_txt[3].' :  ['.$re_m[m_user].']';
...
$f_1b = $lang_lot[113].' : '.number_format(@array_sum($c_total)).' '.$lang_g["text"]["count"];
...
////////////
$fontSize_2 = $fontSize-10;


 $f_settext = $lang_g["_printText"][1];
...
$x_c=_center($im , $fontSize_2,$font,$f_settext);

	imagettftext($im, $fontSize_2, 0, 20 , ($initialY+70)+(($increaseY-2)*(($bantud++)-0)), $black, $font, $f_settext );
...
// imagettftext($im, $fontSize_1, 0, $x_c[c] , $initialY+($increaseY*(($bantud++)-1.5)), $black, $font, $f_2 );

imagettftext($im, $fontSize_1, 0, $x_c[c] ,( $initialY+30)+($increaseY*(($bantud++)-0)), $black, $font, $f_2 );
...
imagettftext($im, $fontSize, 0, $initialX+70, $initialY+($increaseY*$bun), $black, $font, $d_1b);
