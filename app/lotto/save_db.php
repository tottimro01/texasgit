...
###########################SAVE####################################
					   ...
					   , lot_zone	,lot_rob,	lot_type,b_total,	b_pay	,	bill_id,	m_id	,b_date
	                    ...
	                    values ('$time_stam',now(),'$q_num','$re_ok[ok_id]','$zone','$rob' ,'$type_lot' ,'$x_sum', '$adis' 
						 ,'$reeb[bill_id]' ,'".$_SESSION['mid']."','$view_date' 
							...
							,'$my_m' ,'".$my_r1."' 	,'".$my_r2."' 	,'".$my_r3."' 	,'".$my_r4."' 	,'".$my_r5."' 	,'".$my_r6."' 	,'".$my_r7."' 
							  ...
							  ,'$b_web_from', '$bill_cus_name', '$bill_no' ,'$b_dis_arr', '".$_SESSION['cr_id']."' ,'$comaf','$setbet'
								...
								#$_SESSION[error][1][]='<span class="cb">'.$lot_type["th"][$type_lot]." <b>[ "._dt($q_num)." ]</b> = ".number_format($x_sum/$x_sum9)." ตัว : $lang_m[21] </span>";
								 $_SESSION[error][1][]=$lot_type[$lang_post][$_POST[zone]][$type_lot]." [ $q_num ] =".number_format($x_sum/$x_sum9)." ".$lang_member[548]." ".$lang_member[629];
