...
<div class="row">
    <div class="widget-box hidden-boder" id="reloadResult">
        <div class="widget-header widget-header-blue widget-header-flat hidden">
            <h4 class="widget-title lighter"><strong> <?=$lang_soccerResultSoccer[2];?> </strong></h4>
                    ...
                    <div class="col-xs-12 widthTable">
                        <form class="form-horizontal" method="">
                            <div class="form-group">
                                <label class="col-xs-2 col-sm-1 control-label no-padding-right" for=""><?=$lang_soccerResultSoccer[3];?> :</label>
                                <div class="col-xs-4 col-sm-2">
                                    <select class="form-control col-xs-12 col-sm-6 input-sm" id="slSport" onchange="getLeague();">
                                        <option value="sc">ฟุตบอล</option>                                            
                                        <option value="bk">บาสเก็ตบอล</option>                                            
                                        <option value="af">อเมริกันฟุตบอล</option>                                            
                                        <option value="tn">เทนนิส</option>                                            
                                        <option value="ic">ฮ๊อกกี้น้ำแข็ง</option>                                            
                                        <option value="bb">เบสบอล</option>                                            
                                        <option value="vb">วอลเล่ย์บอล</option>                                            
                                        <option value="bt">แบดมินตัน</option>                                            
                                        <option value="my">มวยไทย</option>                                            
                                    </select>
                                </div>
                                <div class="col-xs-6 col-sm-8">
                                    <select class="form-control col-xs-12 col-sm-6 input-sm" id="slLeague" onchange="getMatch();">
                                        <option value="" selected=""><?=$lang_soccerResultSoccer[4];?></option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-xs-2 col-sm-1 no-padding-right" for="username"><?=$lang_soccerResultSoccer[5];?> :</label>
...
<script type="text/javascript">
    ...
    function getLeague(sdate){
        var slLeague = "";
        $.ajax({
            url: 'casinoResultCasinoLeague.php',
            type: 'GET',
            dataType: 'json',
            data: {
        ...
        .done(function(response) {
            slLeague += '<option value="" selected="">ทุกลีค</option>';
            if(response.status){
    ...
    function getMatch(sdate){
        ...
        .done(function(response) {
            ...
            if(response.status){
                ...
                $.each(response.result,function(key,val){
                    tbody += '<tr>'+
                                '<td colspan="'+ response.thead.length +'" class="text-success text-left"><strong>'+ key +'</strong></td>'+
                             '</tr>';
                    switch (sports){
                        case 'sc' : 
                            var win = "";
