...
if($re["r_id"]!=""){
	...
	if($re["r_active"]!=1){
		...
		$_SESSION["bocklogin"]=$_SESSION["bocklogin"]+1;	
			...
			$_SESSION["r_delete_bill"]=$re["r_delete_bill"];
			...
			$sql="update bom_tb_agent set r_login=now(), r_country='$location',	r_ip='$ip'  where r_id='".$_SESSION["rid"]."'";
			sql_query($sql); 
	...
	if($reu["u_id"]!="" and $reu["u_active"]==1){ 
		...
		$_SESSION["r_delete_bill"]=$rer["r_delete_bill"];
        ...
        $sql="update bom_tb_agent_use set u_login=now(),u_ip='$ip' where u_id='".$_SESSION["u_id"]."'";
		sql_query($sql);
