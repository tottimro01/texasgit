    ...
    <div class='form-group'>
      <div class='col-xs-12 col-sm-12 col-md-12'>
        ...
        <div id='setMax_lotto_lao' class='form-inline pull-right'>
          <button type='button' class='btn btn-white btn-warning btn-bold btn-sm' onclick=\"setMax('lotto_lao');\">{$lang_memberAgent[25]}</button>
        </div>
      </div>
    </div>
	    ...
	    <div class='col-xs-12 col-sm-6 col-md-6'>
	        <label class='control-label col-xs-12 col-sm-6 col-md-6 no-padding-right'>
	            <span class='text-danger'>".$array_data["r_name"]."</span>
	                {$lang_memberAgent[27]} : <span id='k_9_lotto_share' data-json='".$r_lotto_hun_set_share[1]."'> ".$array_data["k_9_lotto_share"]."  </span> %
	        </label>
	    </div>
	</div>
                    ...
                    <tbody>";
                        ...
                        $lotto_hun_set_pay_max[$index] =  ($array_data["lotto_hun_set_pay_max"][$index] == "") ? 0 : $array_data["lotto_hun_set_pay_max"][$index];              
                                      ...
                                      <td width='35%' class='lotto_lao_Input'>
                                        ...
                                        <span class='text-danger label-sm maxtransfer' id='lot_hun_set_pay_info' data-json='".$r_lotto_hun_set_pay[$index]."'>(".addCommas($lotto_hun_set_pay_max[$index])." - ".addCommas($r_lotto_hun_set_pay[$index]).") </span>
                                      </td>";
                                    if($index == 1){

                        // $lotto_hun_set_price_max[$index] =  (max($array_data["lotto_hun_set_price_max"][$index]) == "") ? 0 : max($array_data["lotto_hun_set_price_max"][$index]);

                        $lotto_hun_set_price_max[$index] =  ($array_data["lotto_hun_set_price_max"][$index] == "") ? 0 : $array_data["lotto_hun_set_price_max"][$index]; 

                                        $data["temp"] .= "<td width='35%' rowspan='100%' style='vertical-align: top;' class='lotto_lao_Input'>
                                        <input type='text' name='lot_hun_set_price".$index."' id='lot_hun_set_price".$index."' class='numeric maxLimit' value='".$array_data["lot_hun_set_price"][$index]."' onblur=\"chkMinMax(this,".$lotto_hun_set_price_max[$index].",".$r_lotto_hun_set_price[$index].");\">
                                        <span class='text-danger label-sm maxtransfer' id='lot_hun_set_price_info' data-json='".$r_lotto_hun_set_price[$index]."'>(".addCommas($lotto_hun_set_price_max[$index])." - ".addCommas($r_lotto_hun_set_price[$index]).") </span>
                                      </td>";
                                    } 
                                    $data["temp"] .="</tr>";
                                   }
