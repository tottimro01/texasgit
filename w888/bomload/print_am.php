...
$set_sum=array();
$set_sum2=array();
$sql="select  * from  bom_tb_lotto_bill_play_live where m_id='$_GET[mid]' and b_date='$view_date'  and b_accept=1   and (lot_type=4 or lot_type=5  )     order by lot_type asc , play_number asc ";	
 $num=sql_num($sql);
 if($num==0){
...
<span style="float: right;">User : <?=$_GET[user];?> , <?=$_GET[name];?>  งวดที่ :<?=$_GET[d];?></span>
 ...
 <tr align="center" class="tb_title_lotto">
      <td width="22%" ><?=$lang_lot[29];?></td>
      <td width="14%" ><?=$lang_lot[9];?></td>

      <td width="16%" >เล่น</td>
      <td width="16%" ><?=$lang_x[15];?></td>
      <td width="16%" >ถูก</td>
      <td width="16%" ><?=$lang_x[13];?></td>
    </tr>
    <? 
$sum3t=array();
$sum3p=array();
$sum3k=array();
 ...
 $sql="select  *  from  bom_tb_lotto_bill_play_live where    m_id='$_GET[mid]' and b_date='$view_date'  and b_accept=1    and (lot_type!=4 and lot_type!=5 )     order by lot_type asc , play_number asc ";	
  $num=sql_num($sql);
 if($num==0){
