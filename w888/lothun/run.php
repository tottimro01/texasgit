    ...
    <table width="300" border="0" cellspacing="0" cellpadding="0">
      <tbody><tr>
        ...
        <table width="300" border="0" align="center" cellpadding="0" cellspacing="1" class="txt_back11n bg_table">
          ...
          <tr>
            <td height="25" colspan="3" align="center" class="tb_title_lotto"><?=$lang_member[412];?></td>
          </tr>
          <tr>
            <td></td>
            <td align="center"><? if($lot_pay_big5[6]>0){ ?><input name="col1money" type="button" class="btn_le btn_small yellow w50px" id="col1money" value="<?=$lang_member[371];?>" onClick="fcopymoney(7,'tab0')" ><? }?></td>
            <td align="center"><? if($lot_pay_big5[7]>0 and $_SESSION["zone_hun"] !=19){ ?><input name="col2money" type="button" class="btn_le btn_small yellow w50px" id="col2money" value="<?=$lang_member[371];?>" onClick="fcopymoney(8,'tab1')" ><? }?></td>
          </tr>
          <tr class="txt_back12btitle">
            ...
            <td width="110" align="center" bgcolor="#FFFFFF"><table border="0" cellpadding="0" cellspacing="0" class="txt_back12btitle no_cell_border">
              <tbody><tr>
                <td><? if($lot_pay_big5[6]>0){ ?><input name="col1c" type="checkbox" id="col1c" checked="checked" onClick="fdisabled(7,'col1')"><? }?></td>
                <td><?=$lang_member[399];?></td>
              </tr>
            </tbody></table></td>
            <td width="110" align="center" bgcolor="#FFFFFF"><table border="0" cellpadding="0" cellspacing="0" class="txt_back12btitle no_cell_border">
              <tbody><tr>
                <td><? if($lot_pay_big5[7]>0 and $_SESSION["zone_hun"] !=19){ ?><input name="col2c" type="checkbox" id="col2c" checked="checked" onClick="fdisabled(8,'col2')" ><? }?></td>
                <td><?=$lang_member[401];?></td>
              </tr>
            </tbody></table></td>
          </tr>
          <? 
          ...
          <tr>
            ...
            <td align="center" class="bg_td"><? if($lot_pay_big5[6]>0){ ?><input name="col1<?=$i?>" type="text" class="txt_center12n input lotbet 1 _<?=$i?>" data-lotbet-col="1" data-lotbet-row="<?=$i?>" id="tab<?=$tab?>" size="10" maxlength="6" onKeyDown="return numberonly(event,this)"><? } ?></td>
            <td align="center" class="bg_td"><? $tab=$tab+1?><? if($lot_pay_big5[7]>0 and $_SESSION["zone_hun"] !=19){ ?><input name="col2<?=$i?>" type="text" class="txt_center12n input lotbet 2 _<?=$i?>" data-lotbet-col="2" data-lotbet-row="<?=$i?>" id="tab<?=$tab?>" size="10" maxlength="6" onKeyDown="return numberonly(event,this)"><? } ?></td>
          </tr>
          <? $tab=$tab+1; } ?>
          ...
          <tr>
           ...
           <tr>
            <td class="bg_td lotbet_sum_cell text-right lotbet-col-sum total" colspan="3">
...
<div style="padding-top:5px;">
            	...
            	<input name="btsave" type="button" class="btn_le btn_small" id="btsave" value="<?=$lang_member[121];?>" onClick="save_lotto()" <?=$disabled?> >
                                <input name="btreset" type="reset" class="btn_le btn_small danger" id="btreset" value="<?=$lang_member[374];?>" <?=$disabled?> ></div><br>
         	</td>
          </tr>
...
function save_lotto(){
	...
	for(var cc=0;cc<10;cc++){
		...
		nlot=nlot+1;
		if(l3up!="" || l3tod!=""){
				lotto[alot] = num+","+l3up+","+l3tod+",r";
				alot++;
		}
	}
	
		/*var num19h = $.trim($("#num19h").val());
		var up19h = $.trim($("#up19h").val());
		if($("#down19h").attr('disabled')){
			var down19h=  "";
		}else{
			var down19h=  $.trim($("#down19h").val());
		}
		if(num19h!="" && (up19h!="" || down19h!="")){
			if(num19h.length==1){
				lotto[alot] = num19h+","+up19h+","+down19h+",19h";
				alot++;
			}
		}*/
	/*console.log(lotto);
	...
	if(lotto.length<=0){
        ...
        data: { "lotto": lotto , "tlot": 4 , "zone": <?=$_SESSION["zone_hun"];?> , "rob": <?=$_SESSION["rob_hun"];?>, "bill_cus_name": $("#bill_cus_name").val() , "bill_no": $("#bill_no").val() },
				dataType:"json",
