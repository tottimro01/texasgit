...
types[4]=[<? $it1=0; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+3; if($it<99){ echo ","; } } ?>];

types[5]=[<? $it1=1; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+3; if($it<99){ echo ","; } } ?>];

types[6]=[<? $it1=2; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+3; if($it<99){ echo ","; } } ?>];

types[7]=[<? $it1=3; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+4; if($it<99){ echo ","; } } ?>];
...
//alert(fnumber);
	...
	if(fnum!=""){ for(i=1;i<100;i++){ document.getElementById('tab'+types[ty][i]).value = fnum; } }	
...
function create_nums(){
  for(i=0;i<100;i++){ 
    ...
    document.getElementById('tab'+types[4][i]).value = nums; 
  }
}
...
function fdisabled(ty,bcopy){
	...
	if(document.getElementById(bcopy+'c').checked==true){
		...
		for(i=0;i<100;i++){ document.getElementById('tab'+types[ty][i]).className = "txt_center12n"; }

		for(i=0;i<100;i++){ document.getElementById('tab'+types[ty][i]).disabled = false; }
		...
		for(i=0;i<100;i++){ document.getElementById('tab'+types[ty][i]).className = "txt_disabled"; }

		for(i=0;i<100;i++){ document.getElementById('tab'+types[ty][i]).disabled = true; }
...
var myCountdown1 = new Countdown({
      ...
      <input name="col2money" type="button" class="btn_le" id="col2money" value="<?=$lang_member[525];?>" onClick="window.location.href='main_lothun.php?tlot=2ud&zone_send=<?=$_SESSION["zone_hun"];?>&rob_send=<?=$_SESSION["rob_hun"];?>&vvw='+fw" style="width:100px;height:20px;cursor:pointer; margin-top:2px; margin-bottom:2px; line-height:10px; background:#f7c63c; float: left; margin-left: 10px; margin-top: 10px;">
       <input name="col2money" type="button" class="btn_le" id="col2money" value="<?=$lang_member[641];?>" onClick="window.location.href='main_lothun.php?tlot=2ud50&zone_send=<?=$_SESSION["zone_hun"];?>&rob_send=<?=$_SESSION["rob_hun"];?>&vvw='+fw" style="width:100px;height:20px;cursor:pointer; margin-top:2px; margin-bottom:2px; line-height:10px; background:#f7c63c; float: left; margin-left: 10px; margin-top: 10px;">
      </td>
  </tr>
  <tr>
        ...
        <tbody>
          <tr>
            <td valign="top" width="305">
					...
					#	  echo'3333';
					  echo"<center><h2 class='cr'>$lang_member[317]</h1></center>";
					   }elseif($rec["con_open_lotto_hun"]==0 or $check_bet==0){
						#   echo'4444';
					  echo"<center><h2 class='cr'>$lang_member[317]</h1></center>";   
					  }else{
					  ?>   
                  ...
                  <tbody>
                    <tr>
                      ...
                      <table width="300" border="0" align="center" cellpadding="0" cellspacing="1" class="txt_back11n bg_table">
                            ...
                            <tr>
                              <td align="center"><input name="create_num" type="button" class="btn_le" id="create_num" value="<?=$lang_member[527];?>" onClick="create_nums()" style="width:60px;height:20px;cursor:pointer; margin-top:2px; margin-bottom:2px; line-height:10px; background:#f7c63c;"></td>
                              <td align="center"><? if($lot_pay_big5[4]>0){ ?><input name="col1money" type="button" class="btn_le" id="col1money" value="<?=$lang_member[371];?>" onClick="fcopymoney(5,'tab1')" style="width:60px;height:20px;cursor:pointer; margin-top:2px; margin-bottom:2px; line-height:10px; background:#f7c63c;"><? }?></td>
                              <td align="center"><? /*if($lot_pay_big5[5]>0   and $_SESSION["zone_hun"]!=3){*/ ?><input name="col2money" type="button" class="btn_le" id="col2money" value="<?=$lang_member[371];?>" onClick="fcopymoney(6,'tab2')" style="width:60px;height:20px;cursor:pointer; margin-top:2px; margin-bottom:2px; line-height:10px; background:#f7c63c;"><? /*}*/ ?></td>
                            ...
                            <tr class="txt_back12btitle">
                              ...
                              <td width="75" align="center" bgcolor="#FFFFFF"><table border="0" cellpadding="0" cellspacing="0" class="txt_back12btitle no_cell_border">
                                  <tbody>
                                    <tr>
                                      <td><? /*if($lot_pay_big5[5]>0   and $_SESSION["zone_hun"]!=3){*/ ?><?     if($re_ok[o_limit_time_lang]>$time_stam){?><input name="col2c" type="checkbox" id="col2c" checked="checked" onClick="fdisabled(6,'col2')"><? }?><? /*}*/ ?></td>
                                      <td><?=$lang_member[209];?><?#=$lang_member[393];?></td>
                                    </tr>
                                  </tbody>
                                </table></td>
							...
							?>
                            <tr>
                              <td height="22" align="center" class="bg_td"><input name="num<?=$i?>" type="text" class="txt_center12b input" id="tab<?=$tab?>" size="7" maxlength="3" onKeyDown="return numberonly(event,this)" onKeyUp="chtype(<?=$i?>); next_enter(event,<?=$tab?>);"></td>
                              ...
                              <td align="center" class="bg_td"><? $tab=$tab+1?><? /*if($lot_pay_big5[5]>0   and $_SESSION["zone_hun"]!=3){*/ ?><input name="col2<?=$i?>" type="text" class="txt_center12n input" id="tab<?=$tab?>" size="10" maxlength="6" onKeyDown="return numberonly(event,this)" <?     if($re_ok[o_limit_time_lang]<$time_stam){echo' placeholder="'.$lang_member[612].'" disabled="disabled"'; }?> onKeyUp="next_enter(event,<?=$tab?>);"><? /*}*/ ?></td>
...
<div style="padding-top:5px;"><input type="hidden" id="func" name="func">
                                ...
                                <input name="btsave" type="button" class="btn_le" id="btsave" value="<?=$lang_member[121];?> <?=$lang_member[643];?><?=$txtrob;?>" onClick="save_lotto()" <?=$disabled?> style="width:110px;height:20px;cursor:pointer; margin-top:0px; margin-bottom:5px; line-height:10px;">
                                <input name="btreset" type="reset" onClick="load_19h(1)" class="btn_le" id="btreset" value="<?=$lang_member[374];?>" <?=$disabled?> style="width:60px;height:20px;cursor:pointer; margin-top:0px; margin-bottom:5px; line-height:10px; background:#F00;"></div></td>
                            </tr>
                          </tbody>
                      </table></td>
                    </tr>
...
?>
			...
			$.ajax({
				...
				data: { "lotto": lotto , "tlot": <?=$tlot;?> , "zone": <?=$_SESSION["zone_hun"];?> , "rob": <?=$_SESSION["rob_hun"];?> },
				dataType:"json",
			/*	timeout: 30000,*/
