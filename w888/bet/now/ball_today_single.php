...
//$sql="select b_active from pha_tb_ball_list_live;";
//if(sql_query($sql)){$_active = "ok";}else{$_active = "no";}
$a_bb=array();
$url_file="live.json";		
...
<table width="100%" border="0" cellpadding="0" cellspacing="1"   class="ball">
  ...
  <tr align="center" class="txt_white11ntitle" height="25">
    <td width="12%" align="center" bgcolor="#666666">ບ້ານ</td>
    <td width="10%" align="center" bgcolor="#666666">ຢາມ</td>
    <td width="2%" align="center" bgcolor="#666666">%</td>
    <td width="4%" align="center" bgcolor="#666666">ແຕ້ມຕໍ່</td>
    <td width="5%" align="center" bgcolor="#666666">ລາຄາບ້ານ</td>
    <td width="5%" align="center" bgcolor="#666666">ລາຄາຢາມ</td>
    <td width="8%" align="center" bgcolor="#666666">ບ້ານ-ຢາມ</td>
    <td width="4%" align="center" bgcolor="#666666">ສະຫຼຸບ</td>
    ...
    <td width="8%" align="center" bgcolor="#666666">ສູງ-ຕໍ່າ</td>
    <td width="4%" align="center" bgcolor="#666666">ສະຫຼຸບ</td>
    <td width="2%" align="center" bgcolor="#666666">Act</td>
     <? if($_POST["p"]=="config"){ ?>
    <td width="5%" align="center" bgcolor="#666666">Max</td>
    <td width="5%" align="center" bgcolor="#666666">0.02</td>
    <td width="2%" align="center" bgcolor="#666666">New</td>
    <? } ?>
  </tr>
              <? 
			   $os = array();
...
<td colspan="19" bgcolor="#0085cc" class="txt_white11ntitle" style="padding-left:50px;"><span class="txt_white11ntitle" style="padding-left:50px;"><?=$zone_a;?> </span></td>
		 </tr>
             <? 
			 $rball = "";
			 $bg = "#f9f9f9";
...
if($o_id2>0){
	...
	$photobb2='';
		...
		if($timestemp>$time_stam){
		  ...
		  ?>    
 <tr height="34" align="center" <?=$dpd;?> class="row_ball_today tr<?=$idball?>"  >
     <td align="center"   style="background:#<?=$sbg1;?>;">
   <input type="checkbox"   style="float:left;"  onclick="_hit('<?=$idball?>','<?=$add?>','<?=$zone_a;?>',this.value);" value="1"  <? if($_SESSION[hit][$idball][$add]==1){echo'checked="checked"';}?>/>
   ...
   <a href="javascript:_view(4 ,  '<?=$idball;?>' , '<?=$add;?>','<?=$team_1a?>','<?=$team_2a?>');"><?=_mk($sum_3/1000);?></a>&nbsp;</td>
    ...
    <td align="center" style="background:#<?=$sbg17;?>;">
    <input type="text" class="txt_none smax acenter" onClick="c_smax=1;" onKeyDown="return numberonly(event,this,'smax','<?=$add?>','<?=$idball?>','','<?=$smax?>')" id="smax_<?=$idball?><?=$add?>" value="<?=number_format($smax)?>" onkeyup="csetfocus(this,event);"></td>
  
    <td align="center" style="background:#<?=$sbg18;?>;">
    <input type="text" class="txt_none s002 acenter" onClick="c_s002=1;" onKeyDown="return numberonly(event,this,'s002','<?=$add?>','<?=$idball?>','','<?=$s002?>')" id="s002_<?=$idball?><?=$add?>" value="<?=number_format($s002)?>" onkeyup="csetfocus(this,event);"></td>
       
        <td align="center" style="background:#<?=$sbg19;?>;">
        <input type="text" class="txt_none snew acenter" id="snew_<?=$idball?><?=$add?>" onClick="c_snew=1;" onKeyDown="return numberonly(event,this,'snew','<?=$add?>','<?=$idball?>','','<?=$snew?>')" onkeyup="csetfocus(this,event);" value="<?=number_format($snew)?>" maxlength="3"></td>
        
    <? } ?>
    <td><span id="active_check<?=$idball?><?=$add?>"><?php echo $_active;?><a href="javascript:set_active(0,<?php echo $idball;?>,<?php echo $add;?>);" title='ປິດ'><img src="img/del.png"></a></span></td>
          </tr>
          <? } } }  } } ?>
        </tbody>
      </table>
