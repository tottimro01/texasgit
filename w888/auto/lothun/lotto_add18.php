...
# if($x_date!="Sat" and $x_date!="Sun"){
	if($rob>72){
        ...
        $close_time=mktime($hi[hh],$hi[mm],0 ,date("m") ,date("d")+$bdate,date("Y"));
		$close_time_cc=mktime(date("H"),date("i"),0 ,date("m") ,date("d"),date("Y"));
	$o_status = 1;
        $ok_date=date("d-m-Y",$close_time_cc);	  
