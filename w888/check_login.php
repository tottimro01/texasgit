...
$_POST['l_lang']=trim($_REQUEST['clang']);
...
$_SESSION['m_sport_print'] = $re['m_sport_print'];
...
echo"<script language='JavaScript'>var success = 1; top.document.location='index.php?lang=".$_POST['l_lang']."';</script>";
exit();

	
}
}else{
// ------- เช็คว่ามี user มั้ย ถ้ามี เก็บล็อกว่ามีการพยายามเข้าสู่ระบบ
...
$sql="INSERT IGNORE INTO bom_tb_login (h_date,	h_ip	,h_data, h_location	,m_id,r_level, r_agent_id) values(now(),'$ip','$yourbrowser','$location','".$re['m_id']."','5' ,'".$re['m_agent_id']."')";
sql_query($sql);
