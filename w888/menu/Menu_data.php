...
foreach ($M as $key => $value) {
	if(!in_array($key, $use_sports)){
		...
		if($key!='P'){
			// นับจำนวน mixparley ใหม่ บวกจำนวนเฉพาะกีฬาที่เราดึงมาใช้งาน
			$P_E += (isset($M[$key]['E_P'])) ? intval($M[$key]['E_P']) : 0; // ล่วงหน้า
			$P_T += (isset($M[$key]['T_P'])) ? intval($M[$key]['T_P']) : 0; // วันนี้

			// ตัดเมนูย่อยของฟุตบอล
			if($key==1){
				$sc_type = array('E', 'T', 'L', 'E_1X2', 'T_1X2', 'E_OE', 'T_OE', 'E_P', 'T_P'); // เมนูที่เก็บไว้
