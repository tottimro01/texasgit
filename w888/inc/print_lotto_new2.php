...
require("../lang/function_".$lang_post.".php");
...
if($zone==18){
		...
		$name_bill = $lang_g['lotZone'][$zone]." ".$lang_member[643]." ".$lot_robmun[$rob];
	}else{
...
$d[$xss][1] = '['.$lot_type["th"][$rs[lot_type]].'] '.$kkk;
$c_num[]=$xss;
...
#$totalx=number_format(($rs[b_total]/$bet_new)*$emconpay[$rs[lot_type]]);
	$totalt=number_format($rs[b_total]/$bet_new).' '.$lang_member[548];/*'ตัว';*/
