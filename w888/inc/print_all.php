...
#$_SESSION[tang][sum][0]=100;
#$_SESSION[tang][id][0]=15;
?>
<!doctype html>
<html>
<head>
...
<input type="button"  value="<?=$lang_member[377];?>"  onclick="javascript:window.print();window.close();"/>&nbsp;&nbsp;|&nbsp;&nbsp;
<input type="button"  value="<?=$lang_member[612];?>"  onclick="javascript:window.close();"/>
</div>
<p>&nbsp;</p>
...
for($xb=0;$xb<=9;$xb++){
	if($_SESSION[all][sum][$xb]>0){
		$bid=$_SESSION[all][id][$xb];
		
$file="../json/sport/bet_success/".$_SESSION['m_id']."/".$bid.".json";	
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"  class="txt10">
  <tr>
    <td colspan="2" align="center" class="cb">
    <b><?=$lang_member[607];?>:<?=$bm['bill_id'];?>   , <?=$lang_member[643];?>:<?=$bm['b_rob'];?></b>
     <br><span class="cr"><?=$lang_member[146];?></span> <span class="cr"><?=_cvf($bm['b_timestam'],4);?></span></td>
  </tr>
  <tr>
    <td colspan="2" align="center" class="cbu"><?=$lang_member[300];?>  <?=$_SESSION['m_currency'];?> : <span class="cr"> <?=number_format($bm['b_total']);?></span> 
   <!-- <? if($bm['b_total']!=$bm['b_pay']){?>   <?=$lang_member[610];?>  <?=$_SESSION['m_currency'];?> : <span class="cr"><?=number_format($bm['b_total']-$bm['b_pay']);?></span><? }?> -->
    <hr>
<?
...
$leage=_lg($bh['b_zone'], $sport_lang[Zone][md5($bh['b_zone'])]);
 ...
 $team_2= _lg($bh['b_name_2'], $sport_lang[Team][md5($bh['b_name_2'])]);				
  ...
  <? $x++;}?>
    ...
    <span style="float: left;"><?=$lang_member[767];?> <b><?=$bm['b_numstep'];?></b></span>
    <strong><?=$lang_member[1039];?>   :  <?=$_SESSION['m_currency'];?>  : <span class="cr"> <?=number_format($bm['b_total']*$bm['b_sum_pay']);?> </span></strong></td>
  </tr>
</table>
