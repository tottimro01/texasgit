...
$sql = sql_query("select * from bom_tb_football_bill_live where m_id = '".$_SESSION['m_id']."' and b_running = 0 and b_status = 5 order by bill_id desc limit 5");
while($rs=sql_fetch($sql)){
	if($rs["b_numstep"]>1){
?>
<div class="BetInfo <? if($rs["b_accept"]==2){ echo "canlight"; }else{ echo "bgcpelight"; } ?>" onMouseOver="this.className='BetInfo Ltrbgov'" onMouseOut="this.className='BetInfo <? if($rs["b_accept"]==2){ echo "canlight"; }else{ echo "bgcpelight"; } ?>'">
...
<div class="TextStyle01 <? if($rs["b_accept"]==2){ echo "cantext"; } ?>"><span class="TextStyle03">@</span><span class="UdrDogOddsClass"><?=number_format($rs["b_sum_pay"],2)?></span></div>
    ...
    <div class="multiple-box" style="border-bottom: #b1b1b1 1px solid;">
        ...
        <div class="multiple">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td class="TextStyle08 singleTicketStatus <? if($rs["b_accept"]==2){ echo "cantext"; } ?>" style="text-align: right;"><? if($rs["b_accept"]==2){ echo $lang_member[1422]; }else{ echo $lang_member[956]; } ?></td>
                </tr>
            </table>
        </div>
        <?
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" style="padding-top: 5px;">
        ...
        <tr>
            <td colspan="2" class="TextStyle06"><div class="left"><img src="img/printer.png" onclick="print_bill('<?=$rs["bill_id"];?>');" style="vertical-align: middle; padding-right: 5px; cursor: pointer;">ID:<?=$rs["bill_id"]?></div><div class="right text-ellipsis statusInfo <? if($rs["b_accept"]==2){ echo "cantext"; } ?>" style="width:100px; text-align:right; line-height:inherit;" title="<? if($rs["b_accept"]==2){ echo $lang_member[1422]; }else{ echo $lang_member[956]; } ?>"><? if($rs["b_accept"]==2){ echo $lang_member[1422]; }else{ echo $lang_member[956]; } ?></div></td>
        </tr>
    </table>
    </div>
</div>
...
?>	
	<div class="BetInfo <? if($rs["b_accept"]==2){ echo "canlight"; }else{ echo "bgcpe"; } ?>" onMouseOver="this.className='BetInfo Ltrbgov'" onMouseOut="this.className='BetInfo <? if($rs["b_accept"]==2){ echo "canlight"; }else{ echo "bgcpe"; } ?>'"> 
...
<span class="" style="background: yellow; padding: 0px 2px;">[<?=str_replace(' ', '', $rs_b["b_score_live"])?>] <?=$rs_b["b_time_play"]?>'</span>
    ...
    <tr>
        <td colspan="2" class="TextStyle06"><div class="left"><img src="img/printer.png" onclick="print_bill('<?=$rs["bill_id"];?>');" style="vertical-align: middle; padding-right: 5px; cursor: pointer;">ID:<?=$rs["bill_id"]?></div><div class="right statusInfo <? if($rs["b_accept"]==2){ echo "cantext"; } ?>" style="width:100px; text-align:right; line-height:inherit;" title="<? if($rs["b_accept"]==2){ echo $lang_member[1422]; }else{ echo $lang_member[956]; } ?>"><? if($rs["b_accept"]==2){ echo $lang_member[1422]; }else{ echo $lang_member[956]; } ?></div></td>
    </tr>
  </table>
...
if($nl==0){
	?>
<div style="padding: 4px;"><div align="center"><?=$lang_member[1421]?></div><div align="center" id="lastrefresh" style="display: none">{{lbl_countDown}} : <span id="lastrefresh_time"></span></div><div align="center" id="checkStatus" style="display: none">{{lbl_checkStatus}}</div>   </div>
	<?
}
?>