...
$_SESSION[error][1][] = $lot_type[$lang_post][$_POST[zone]][$type_lot] . " [ " . _dt($q_num) . " ] = จำกัดเลข ".$q_num." :  ". round($jxstop[sum]/14.29)."ตัว/ครั้ง";
		 $_SESSION[error][2][] = 88;
		 ########################ประวัติเลขเต็ม
 include("save_topmax.php");
  ########################ประวัติเลขเต็ม
		 }else{
$_SESSION[error][1][] = $lot_type[$lang_post][$_POST[zone]][$type_lot] . " [ " . _dt($q_num) . " ] = จำกัดเลข ".$q_num." :  ". $jxstop[sum]." บาท/ครั้ง";
		 $_SESSION[error][2][] = 88;
		 ########################ประวัติเลขเต็ม
 include("save_topmax.php");
  ########################ประวัติเลขเต็ม
...
# $_SESSION[error][1][]='<span class="cr">จำกัดเลข'.$q_num.' :  ชั่วคราว</span>';	
  $_SESSION[error][1][] = $lot_type[$lang_post][$_POST[zone]][$type_lot] . " [ " . _dt($q_num) . " ] = จำกัดเลข ".$q_num." : รอ ". _fbt($jxstop[xnext]-$time_stam)."นาที";
	 $_SESSION[error][2][] = 77;
 ########################ประวัติเลขเต็ม
 include("save_topmax.php");
  ########################ประวัติเลขเต็ม
...
#$_SESSION[error][1][]='<span class="cr">'.$q_num.' เหลือแทง  : '.number_format($jxstop[sum]).'</span>';	
   $_SESSION[error][1][] = $lot_type[$lang_post][$_POST[zone]][$type_lot] . " [ " . _dt($q_num) . " ] = ".$q_num." เหลือแทง  : ".number_format($jxstop[sum])."";
	 $_SESSION[error][2][] = 66;
########################ประวัติเลขเต็ม
 include("save_topmax.php");
  ########################ประวัติเลขเต็ม
...
#######################################################LOG
	...
	#######################################################LOG
								...
								#$_SESSION[error][1][]='<span class="cb">'.$lot_type["th"][$type_lot]." <b>[ "._dt($q_num)." ]</b> = ".number_format($x_sum/$x_sum9)." ตัว : $lang_m[21] </span>";
								 $_SESSION[error][1][]=$lot_type[$lang_post][$_POST[zone]][$type_lot]." [ $q_num ] =".number_format($x_sum/$x_sum9)." ตัว ".$lang_m[21];
									// $_SESSION[error][1][]=$lot_type["th"][$type_lot]." [ $q_num ] =".number_format($x_sum)." ตัว ".$lang_m[21];
