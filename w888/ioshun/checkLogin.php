...
if($re[m_id]!="" and $re[m_active]==1){
	...
	if($strdevice=="ios"){
		...
		$sql_uid = sql_query("update bom_tb_member set 	m_uid_hun_ios = '$uid' , m_uuid_ios = '$uuid' where m_id = '$re[m_id]'");
		...
		$sql_uid = sql_query("update bom_tb_member set m_uid_hun_android = '$uid' , m_uuid_android = '$uuid' where m_id = '$re[m_id]'");
	}
...
#$yourbrowser= $ua['name'] . "#" . $ua['version'] . "#" .$ua['platform2'];
 $sql="INSERT IGNORE INTO bom_tb_login (h_date,	h_ip	,h_data, h_location	,m_id,r_level, r_agent_id) values(now(),'$ip','App','$location','$re[m_id]','5' ,'$re[r_agent_id]')";
 sql_query($sql);
...
$arr["log"] = $url_file;
	...
	$arr["lot_pay_big1"] =str_replace(',,',',0,', $lot_m["m_lotto_hun_pay_super"]);
	$arr["lot_pay_big2"] = str_replace(',,',',0,',$lot_m["m_lotto_hun_pay_senior"]);
	$arr["lot_pay_big3"] = str_replace(',,',',0,',$lot_m["m_lotto_hun_pay_master"]);
	$arr["lot_pay_big4"] = str_replace(',,',',0,',$lot_m["m_lotto_hun_pay_agent"]);
	$arr["lot_pay_big5"] = str_replace(',,',',0,',$lot_m["m_lotto_hun_pay_member"]);
	...
	$arr["Hunset"] = "$lothun_set";

	$exp=@explode(',',$arr["lot_pay_big5"]);
