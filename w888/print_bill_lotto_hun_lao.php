...
$sql="select * from bom_tb_lotto_bill_play_live where lot_zone='".$_GET['id']."' and m_id='".$_SESSION['m_id']."'  and b_accept=1 ";
$ree=sql_array($sql);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"     >
  <tr>
    ...
    <b><?=$arr_zone[$ree['lot_zone']];?> <?=$lang_g["laoset"]?> <?=$lang_member[706];?>  <?=$ree['bill_id'];?></b>
     <br><b><?=$lang_member[569];?> <?=_cvf($ok_date['o_limit_time'],5 );?> 
  ...
  <?=$lang_member[688];?> <?=$ree['lot_rob'];?>
		...
		<? }else{ ?>
		<?=$lang_member[643];?><?=$lot_rob[$ree['lot_rob']];?>
		<? } } ?>
     <br><?=$lang_member[146];?> <span class="cr"><?=_cvf($ree['b_timestam'],4);?></span>
...
$sum[]=ceil( $rs['b_total']); 
  ...
  <tr <?  //$bgc = ($bgc=="#fff") ? "#e3e3e3" : "#fff"; {echo"style='background:$bgc'";}?> class="cb" <? if($rs["play_status"]==1){ echo "bgcolor='#FF9900'";} ?>>
    ...
    <b style="float:right;">--<?=number_format(1)." ".$lang_g["text"]["count"];?>--</b>    </td>
  </tr>
  <? 
  $c_num[]=$xss;
   $xss++; } ?>
...
<br style="clear:both;">
<br>
<b style=" float:right;"><?=$lang_member[559]?> <span class="cr"> <?=number_format(count($c_num));?></span> <?=$lang_g["text"]["count"]?></b>
<br>
<br style="clear:both;">
<br>
<div class="sb2" ></div>
