...
var _c=15;
var _t;
function Today_Basketball_List(){
	c=91;
	_c=15;
	clearTimeout(t);
	clearTimeout(_t);
	...
	BasketballToday.location="data/sport_data.php?sc=bkt&t="+Math.random();
}
...
function load_today(file_req , nload){
	var url;
	if(file_req==1){
		url = "inc/line_data.php?sc=bkt1l&t="+Math.random();
	}else if(file_req==2){
		url = "inc/line_data.php?sc=bkt2l&t="+Math.random();
	}else if(file_req==3){
		url = "inc/line_data.php?sc=bktft&t="+Math.random();
	}else if(file_req==4){
		url = "inc/line_data.php?sc=bkt1h&t="+Math.random();
	}else if(file_req==5){
		url = "inc/line_data.php?sc=bkt1x2&t="+Math.random();
	}else if(file_req==6){
		url = "inc/line_data.php?sc=bktoe&t="+Math.random();
	}
	$.ajax({
		type: "POST",
		url: url,
...
function timedCount(lv)
{
	...
	if (c <= 0){ 
		...
		if((c==15 || c==30 || c==45 || c==60 || c==75) && lv == 1){ 
			Today_Basketball_Refresh(); 
		}
	}
}

function Today_Basketball_Refresh(){ 
	if(document.getElementById("main_today").innerHTML != ""){
		BasketballTodayRefresh.location ="data/sport_data.php?sc=bktr&t="+Math.random();
	}	
}
