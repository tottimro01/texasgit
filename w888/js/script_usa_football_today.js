...
var _c=15;
var _t;
function Today_USA_football_List(){
	c=91;
	_c=15;
	clearTimeout(t);
	clearTimeout(_t);
	...
	USA_footballToday.location="data/sport_data.php?sc=ust&t="+Math.random();
}
...
function load_today(file_req , nload){
	var url;
	if(file_req==1){
		url = "inc/usa_football_today_singleLine.php?t="+Math.random();
	}else if(file_req==2){
		url = "inc/usa_football_today_twoLine.php?t="+Math.random();
	}else if(file_req==3){
		url = "inc/usa_football_today_fullTime.php?t="+Math.random();
	}else if(file_req==4){
		url = "inc/usa_football_today_halfTime.php?t="+Math.random();
	}else if(file_req==5){
		url = "inc/usa_football_today_1x2.php?t="+Math.random();
	}else if(file_req==6){
		url = "inc/usa_football_today_kk.php?t="+Math.random();
	}
	$.ajax({
		type: "POST",
		url: url,
...
function timedCount(lv)
{
	...
	if (c <= 0){ 
		...
		if((c==15 || c==30 || c==45 || c==60 || c==75) && lv == 1){ 
			Today_USA_football_Refresh(); 
		}
	}
}

function Today_USA_football_Refresh(){ 
	if(document.getElementById("main_today").innerHTML != ""){
		USA_footballTodayRefresh.location ="data/sport_data.php?sc=ustr&t="+Math.random();
	}	
}
