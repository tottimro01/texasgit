...
function Today_Snooker_List(){
	...
	SnookerToday.location="data/sport_data.php?sc=skt&t="+Math.random();
}
...
function load_today(file_req , nload){
	var url;
	if(file_req==1){
		url = "inc/snooker_today_singleLine.php?t="+Math.random();
	}else if(file_req==2){
		url = "inc/snooker_today_twoLine.php?t="+Math.random();
	}else if(file_req==3){
		url = "inc/snooker_today_fullTime.php?t="+Math.random();
	}else if(file_req==4){
		url = "inc/snooker_today_halfTime.php?t="+Math.random();
	}else if(file_req==5){
		url = "inc/snooker_today_1x2.php?t="+Math.random();
	}else if(file_req==6){
		url = "inc/snooker_today_kk.php?t="+Math.random();
	}
	$.ajax({
		type: "POST",
		url: url,
...
function timedCount(lv)
{
	...
	if (c <= 0){ 
		...
		if((c==15 || c==30 || c==45 || c==60 || c==75) && lv == 1){ 
			Today_Snooker_Refresh(); 
		}
	}
}

function Today_Snooker_Refresh(){ 
	if(document.getElementById("main_today").innerHTML != ""){
		SnookerTodayRefresh.location ="data/sport_data.php?sc=sktr&t="+Math.random();
	}	
}
