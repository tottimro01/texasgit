...
var _t2;
var Nl2 = [];
function Live_USA_football_List(){
	 c2=30;
	_c2=15;
	clearTimeout(t2);
	clearTimeout(_t2);
	...
	USA_footballLive.location="data/sport_data.php?sc=usl&t="+Math.random();
}
...
function load_live(file_req , nload){
	var url;
	if(file_req==1){
		url = "inc/usa_football_live_singleLine.php?t="+Math.random();
	}else if(file_req==2){
		url = "inc/usa_football_live_twoLine.php?t="+Math.random();
	}else if(file_req==3){
		url = "inc/usa_football_live_fullTime.php?t="+Math.random();
	}else if(file_req==4){
		url = "inc/usa_football_live_halfTime.php?t="+Math.random();
	}else if(file_req==5){
		url = "inc/usa_football_live_1x2.php?t="+Math.random();
	}
...
function timedCount2(lv)
{
	...
	if (c2 <= 0){ 
		...
		if((c2==10 || c2==20) && lv == 1){ 
			Live_USA_football_Refresh(); 
		}
	}
}

function Live_USA_football_Refresh(){ 
	if(document.getElementById("main_live").innerHTML != ""){
		USA_footballLiveRefresh.location ="data/sport_data.php?sc=uslr&t="+Math.random();
	}	
}
