...
function Live_Snooker_List(){
	...
	SnookerLive.location="data/sport_data.php?sc=skl&t="+Math.random();
}
...
function load_live(file_req , nload){
	var url;
	if(file_req==1){
		url = "inc/snooker_live_singleLine.php?t="+Math.random();
	}else if(file_req==2){
		url = "inc/snooker_live_twoLine.php?t="+Math.random();
	}else if(file_req==3){
		url = "inc/snooker_live_fullTime.php?t="+Math.random();
	}else if(file_req==4){
		url = "inc/snooker_live_halfTime.php?t="+Math.random();
	}else if(file_req==5){
		url = "inc/snooker_live_1x2.php?t="+Math.random();
	}
...
function timedCount2(lv)
{
	...
	if (c2 <= 0){ 
		...
		if((c2==10 || c2==20) && lv == 1){ 
			Live_Snooker_Refresh(); 
		}
	}
}

function Live_Snooker_Refresh(){ 
	if(document.getElementById("main_live").innerHTML != ""){
		SnookerLiveRefresh.location ="data/sport_data.php?sc=sklr&t="+Math.random();
	}	
}
