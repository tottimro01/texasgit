...
types[1]=[<? $it1=1; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+4; if($it<99){ echo ","; } } ?>];

types[2]=[<? $it1=2; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+4; if($it<99){ echo ","; } } ?>];

types[3]=[<? $it1=3; for($it=0;$it<100;$it++){ echo $it1; $it1=$it1+4; if($it<99){ echo ","; } } ?>];
...
function fnumcrossx(){
	...
	if(fnum.length==3){	
		for(i=1;i<100;i++){ document.getElementById('tab'+(i*4)).value = ''; }	
		var vu = [];
		var org_vu = [];	
...
function fdisabled(ty,bcopy){
    ...
    for(var i=0;i<100;i++){ 
		...
		// for(i=0;i<100;i++){ document.getElementById('tab'+types[ty][i]).disabled = false; chtype(i) }
    ...
    for(var i=0;i<100;i++){ 
		...
		// for(i=0;i<100;i++){ document.getElementById('tab'+types[ty][i]).disabled = true; chtype(i) }
...
var myCountdown1 = new Countdown({
      ...
      <td><input type="button" class="btn_le btn_small" value="<?=$lang_member[525]?>" onClick="window.location.href='main_lotto.php?tlot=&vvw='+fw"></td>
      ...
      <td>
        <table width="100%" cellspacing="2" cellpadding="0" class="lotto_open_table">
          ...
          <tr>
            <td align="center"><?=$day_open?></td>
            <td align="center"><?=$time_close1?></td>
            <td align="center"><?=$time_close2?></td>
          </tr>
        </table>
      </td>
  </tr>
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.res==1){
						...
						for(i=0;i<100;i++){ chtype(i);$("#chk6g"+i).prop("checked" , false); }
