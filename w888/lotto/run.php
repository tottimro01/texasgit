    ...
    <table width="300" border="0" cellspacing="0" cellpadding="0">
      <tbody><tr>
        ...
        <table width="300" border="0" align="center" cellpadding="0" cellspacing="1" class="txt_back11n bg_table">
          ...
          <tr>
            <td></td>
            <td align="center"><? if($lot_pay_big5[6]>0){ ?><input name="col1money" type="button" class="btn_le btn_small w50px yellow" id="col1money" value="<?=$lang_member[371];?>" onClick="fcopymoney(7,'tab0')" ><? }?></td>
            <td align="center"><? if($lot_pay_big5[7]>0){ ?><input name="col2money" type="button" class="btn_le btn_small w50px yellow" id="col2money" value="<?=$lang_member[371];?>" onClick="fcopymoney(8,'tab1')" ><? }?></td>
          </tr>
          <tr class="txt_back12btitle">
            ...
            <td width="110" align="center" bgcolor="#FFFFFF"><table border="0" cellpadding="0" cellspacing="0" class="txt_back12btitle no_cell_border">
              <tbody><tr>
                <td><? if($lot_pay_big5[7]>0){ ?><?     if($re_ok['o_limit_time_lang']>$time_stam){?><input name="col2c" type="checkbox" id="col2c" checked="checked" onClick="fdisabled(8,'col2')" ><? }?><? }?></td>
          ...
          <tr>
            ...
            <td align="center" class="bg_td"><? if($lot_pay_big5[6]>0){ ?><input name="col1<?=$i?>" type="text" class="txt_center12n input lotbet 1 _<?=$i?>" data-lotbet-col="1" data-lotbet-row="<?=$i?>" id="tab<?=$tab?>" size="10" maxlength="6" onKeyDown="return numberonly(event,this)" onKeyUp="next_enter(event,<?=$tab?>);"><? } ?></td>
            <td align="center" class="bg_td"><? $tab=$tab+1?><? if($lot_pay_big5[7]>0){ ?><input name="col2<?=$i?>" type="text" class="txt_center12n input lotbet 2 _<?=$i?>" data-lotbet-col="2" data-lotbet-row="<?=$i?>" id="tab<?=$tab?>" size="10" maxlength="6" onKeyDown="return numberonly(event,this)" <?     if($re_ok['o_limit_time_lang']<$time_stam){echo' placeholder="'.$lang_member[612].'" disabled="disabled"'; }?> onKeyUp="next_enter(event,<?=$tab?>);"><? } ?></td>
          </tr>
          <? $tab=$tab+1; } ?>
          ...
          <tr>
          	<td colspan="3" class="bg_td lotbet_sum_cell text-right lotbet-col-sum total">
	...
	if(lotto.length<=0){
		...
		if (rcc == true) {	
			$.ajax({
				...
				data: { "lotto": lotto , "tlot": 4 , "zone": 1 , "rob": 1 , "bill_cus_name": $("#bill_cus_name").val() , "bill_no": $("#bill_no").val() },
				dataType:"json",
