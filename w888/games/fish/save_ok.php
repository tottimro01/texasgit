...
echo"<b class='cr'>$lang_game[73]</b>";
	exit();
	}
...
if($re_m['m_count']<=0){
	echo"<b class='cr'>$lang_game[81] : $re_m[m_count] $lang_game[78]</b>";
	exit();
}
...
foreach($_SESSION['arid'] as $rid){
					...
					echo "<b class='cr'>E32-".$lang_lot[110]."</b>";
 @session_start(); 
@session_destroy();
					 exit();
					 }
...
sql_query($sql);	
	...
	'$re_m[m_count]' ,'-$sumbet','$log_sum','สมาชิกพนันเกมส์ น้ำเต้าปูปลา',
...
foreach($exx as &$values){
	...
	$sql="update IGNORE  bom_tb_games_bet_live set  bet_count_$values=bet_count_$values +$vals    where bet_id='$bet_id'  and game_zone='$zone'";
	sql_query($sql);
