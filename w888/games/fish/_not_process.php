...
$bet_id=$date2_beti[0][id];
	...
	###########################คืนเงิน
$sql="select * from pha_tb_fish_bill_play where play_status='7' and  bill_id!='$bet_id'   and lock_pay=0  order by play_id desc";
$re=sql_query($sql);
while($rs=sql_fetch($re)){
$sumbet=$rs[b_total];
echo'<br>'.$sql="update pha_tb_member set m_count=m_count+$sumbet  where m_id='$rs[m_id]' ";
sql_query($sql);	
echo'<br>'.$sql="update pha_tb_fish_bill_play set lock_pay=1, play_status=3 , b_bonus=$sumbet,  br_bonus=$sumbet  where play_id='$rs[play_id]'  ";
sql_query($sql);
