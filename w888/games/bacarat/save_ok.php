...
if($rag[0]>0){$paywin1=$arr_bacarat_pay[$type];}
if($rag[1]>0){$paywin2=$arr_bacarat_pay[$type];}
if($rag[2]>0){$paywin3=$arr_bacarat_pay[$type];}
if($rag[3]>0){$paywin4=$arr_bacarat_pay[$type];}
if($rag[4]>0){$paywin5=$arr_bacarat_pay[$type];}
if($rag[5]>0){$paywin6=$arr_bacarat_pay[$type];}
if($rag[6]>0){$paywin7=$arr_bacarat_pay[$type];}
if($rag[7]>0){$paywin8=$arr_bacarat_pay[$type];}
$paywinh=$arr_bacarat_pay[$type];
...
sql_query($sql);	
	...
	'$re_m[m_count]' ,'-$sumbet','$log_sum','สมาชิกพนันเกมส์ บาคาร่า',
...
/*
################################ค่าแนะนำสมาชิก
$sql="select *  from bom_tb_casino_bill_play   where bet_id='$bet_id'  and m_from='$_SESSION[m_from]'  order by  play_id desc  limit 1 ";
$rex=sql_array($sql);
	$xx_bonus=$sumbet*(2/100);
	$sql="insert into bom_tb_from_bonus_bacarat  (	play_id,	b_date_bingo	,b_total	,fb_com,	m_id	,m_from, bill_id) values ( '$rex[play_id]',now(),'$sumbet','$xx_bonus','$rex[m_id]' ,'$_SESSION[m_from]','$bet_id') ";
	sql_query($sql);
	#######################################################อัพเดทเงินสมาชิก
...
$js1=array();
$js1[]=array("1"=>"$rec[bet_count_1]","2"=>"$rec[bet_count_2]","3"=>"$rec[bet_count_3]","4"=>"$rec[bet_count_4]","5"=>"$rec[bet_count_5]","6"=>"$rec[bet_count_6]","7"=>"$rec[bet_count_7]");
