...
if($rag[0]>0){$paywin1=$arr_dragon_pay[$type];}
if($rag[1]>0){$paywin2=$arr_dragon_pay[$type];}
if($rag[2]>0){$paywin3=$arr_dragon_pay[$type];}
if($rag[3]>0){$paywin4=$arr_dragon_pay[$type];}
if($rag[4]>0){$paywin5=$arr_dragon_pay[$type];}
if($rag[5]>0){$paywin6=$arr_dragon_pay[$type];}
if($rag[6]>0){$paywin7=$arr_dragon_pay[$type];}
if($rag[7]>0){$paywin8=$arr_dragon_pay[$type];}
$paywinh=$arr_dragon_pay[$type];
...
sql_query($sql);	
	...
	'$re_m[m_count]' ,'-$sumbet','$log_sum','สมาชิกพนันเกมส์ ดราก้อน-ไทเกอร์',
...
$js1=array();
$js1[]=array("1"=>"$rec[bet_count_1]","2"=>"$rec[bet_count_2]","3"=>"$rec[bet_count_3]");
