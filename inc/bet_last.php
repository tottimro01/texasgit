...
$url_file4="../json/sport/bet_success/".$_SESSION['mid']."/*.json";	
$x_del=(60*60*4*1);
$files = @glob($url_file4); // get all file names
@array_multisort(@array_map('filemtime', $files), SORT_NUMERIC, SORT_DESC, $files);
foreach(@array_slice($files, 0, 3) as $file){ // iterate files
  if(@is_file($file)){
$ct=@filemtime($file);
$nt=$time_stam-$ct;
...
?>
  ...
  <tr>
	...
	?>
        ...
        <tr>
    <td colspan="2" align="right" ><strong class="blue2"><?=$del;?><?=$dd['play_bet'];?></strong> @<b><?=_cbp($dd['play_pay'],3);?></b> <?=$m_price[$dd['m_price']];?> <span class="green"><?=$ab_bet[$dd['b_accept']];?></span></td>
    </tr>
        <tr>
    <td class="sb"></td>
    <td class="sb"></td>
...
<? }?> 
    ...
    <tr bgcolor="#CCCCCC">
       <td align="center"  class="green"><strong><?=$ab_bet[$bd['b_accept']];?></strong></td>
...
var ha_last = 30;
chktime_last();
function chktime_last(){
	ha_last = ha_last-1;
	if(ha_last==0){
		if (tmc_last) { clearTimeout(tmc_last); }
		menu_bet(2,0);
	}else{
		//console.log(ha_last , "l");
		if (tmc_last) { clearTimeout(tmc_last); }
		tmc_last=setTimeout("chktime_last()",1000);
	}
}
</script>